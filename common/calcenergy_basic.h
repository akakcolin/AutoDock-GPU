...
// Original
#define GETGRIDVALUE(mempoi, gridsize_x, gridsize_y, gridsize_z, t, z, y, x) \
  *(mempoi + gridsize_x * (y + gridsize_y * (z + gridsize_z * t)) + x)

// Optimization 1
// #define GETGRIDVALUE_OPT(mempoi,gridsize_x,gridsize_y,mul_tmp,z,y,x) *(mempoi
// + gridsize_x*(y + gridsize_y*(z + mul_tmp)) + x)
...
#define TRILININTERPOL(cube, weights)                                    \
  (cube[idx_000] * weights[idx_000] + cube[idx_010] * weights[idx_010] + \
   cube[idx_001] * weights[idx_001] + cube[idx_011] * weights[idx_011] + \
   cube[idx_100] * weights[idx_100] + cube[idx_110] * weights[idx_110] + \
   cube[idx_101] * weights[idx_101] + cube[idx_111] * weights[idx_111])
...
#define STEP_START \
  1E3  // Starting step size. This might look gigantic but will cap
#define MAX_DEV_TRANSLATION \
  2.0f  // 2 Angstrom, but must be divided by the gridspacing (store in
        // variable)
