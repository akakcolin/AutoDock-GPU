...
enum {
  ...
  S = 6,
  F = 7
};  // see "bond_index" in the "AD4.1_bound.dat" or "AD4_parameters.dat" file.
#define NUM_ENUM_ATOMTYPES \
  8  // this should be the length of the enumerated atom types above

// Indexes of atomic types used in
// host/src/processligand.cpp/get_VWpars(),
// and kernel energy & gradient calculation.
#define ATYPE_NUM \
  28  // 22 (initial) + 2 (CG & G0 for handling flexrings) + W (waters) + CX +
      // NX + OX
