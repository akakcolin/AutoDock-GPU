...
#define CONVERT_INTO_ANGSTROM_RADIAN  // DO NOT UNDEFINE, NO REALLY! DO!!!
                                      // NOT!!! UNDEFINE!!! SML 200608
...
__device__ void gpu_calc_energrad(float* genotype, float& global_energy,
                                  int& run_id,
...
// In Gradient-Minimizer: must calculate gradients
  ...
  // Derived from autodockdev/maps.py
  for (uint32_t atom_id = threadIdx.x;atom_id <cData.dockpars.num_of_atoms;  // makes sure that gradient sum reductions give
  ...
  for (uint32_t gene_cnt = threadIdx.x; gene_cnt < cData.dockpars.num_of_genes;
       gene_cnt += blockDim.x) {
  ...
  for (uint32_t rotation_counter = threadIdx.x;
    ...
    int rotation_list_element =
        cData.pKerconst_rotlist->rotlist_const[rotation_counter];
          ...
          0)  // If rotating around rotatable bond
      {
        ...
        rotation_unitvec.x =
            s * cData.pKerconst_conform
                    ->rotbonds_unit_vectors_const[3 * rotbond_id];
        rotation_unitvec.y =
            s * cData.pKerconst_conform
        ...
        rotation_movingvec.x =
            cData.pKerconst_conform
                ->rotbonds_moving_vectors_const[3 * rotbond_id];
  ...
  float weights[8];
  float cube[8];
  for (uint32_t atom_id = threadIdx.x; atom_id < cData.dockpars.num_of_atoms;
       atom_id += blockDim.x) {
    ...
    uint32_t atom_typeid =
        cData.pKerconst_interintra->atom_types_map_const[atom_id];
...
#else
      gradient[atom_id].x +=
          lrintf((TERMSCALE * 42.0f * x) / cData.dockpars.grid_spacing);
      gradient[atom_id].y +=
          lrintf((TERMSCALE * 42.0f * y) / cData.dockpars.grid_spacing);
      gradient[atom_id].z +=
          lrintf((TERMSCALE * 42.0f * z) / cData.dockpars.grid_spacing);
    ...
    // Grid value at 000
    float* grid_value_000 =
        cData.pMem_fgrids + ((ulong)(x_low + y_low * g1 + z_low * g2) << 2);
    ...
    */
    ...
    // Calculating affinity energy
    energy += q * (cube[0] * weights[0] + cube[1] * weights[1] +
                   cube[2] * weights[2] + cube[3] * weights[3] +
                   cube[4] * weights[4] + cube[5] * weights[5] +
                   cube[6] * weights[6] + cube[7] * weights[7]);
#if defined(DEBUG_ENERGY_KERNEL)
    interE += q * (cube[0] * weights[0] + cube[1] * weights[1] +
                   cube[2] * weights[2] + cube[3] * weights[3] +
                   cube[4] * weights[4] + cube[5] * weights[5] +
                   cube[6] * weights[6] + cube[7] * weights[7]);
#endif
    ...
    // Vector in x-direction
    gx += q1 * (omdz * (omdy * (cube[idx_100] - cube[idx_000]) +
                        dy * (cube[idx_110] - cube[idx_010])) +
                dz * (omdy * (cube[idx_101] - cube[idx_001]) +
                      dy * (cube[idx_111] - cube[idx_011])));
    // Vector in y-direction
    gy += q1 * (omdz * (omdx * (cube[idx_010] - cube[idx_000]) +
                        dx * (cube[idx_110] - cube[idx_100])) +
                dz * (omdx * (cube[idx_011] - cube[idx_001]) +
                      dx * (cube[idx_111] - cube[idx_101])));
    // Vectors in z-direction
    gz += q1 * (omdy * (omdx * (cube[idx_001] - cube[idx_000]) +
                        dx * (cube[idx_101] - cube[idx_100])) +
                dy * (omdx * (cube[idx_011] - cube[idx_010]) +
                      dx * (cube[idx_111] - cube[idx_110])));
    // -------------------------------------------------------------------
...
#else
    gradient[atom_id].x +=
        lrintf(fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * gx)));
    gradient[atom_id].y +=
        lrintf(fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * gy)));
    gradient[atom_id].z +=
        lrintf(fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * gz)));
#endif
  ...
  // ================================================
#ifdef REPRO
  // Simplest way to ensure random order of atomic addition doesn't make answers
  // irreproducible: use only 1 thread
  if (threadIdx.x == 0)
    for (uint32_t contributor_counter = 0;
  ...
  for (uint32_t contributor_counter = threadIdx.x;
      ...
      // Getting type IDs
      uint32_t atom1_typeid =
          cData.pKerconst_interintra->atom_types_const[atom1_id];
      uint32_t atom2_typeid =
          cData.pKerconst_interintra->atom_types_const[atom2_id];

      uint32_t atom1_type_vdw_hb =
          cData.pKerconst_intra->atom1_types_reqm_const[atom1_typeid];
      uint32_t atom2_type_vdw_hb =
          cData.pKerconst_intra->atom2_types_reqm_const[atom2_typeid];
      ...
      if (atomic_distance < 8.0f) {
        ...
        float opt_distance =
            (cData.pKerconst_intra
                 ->reqm_const[atom1_type_vdw_hb + ATYPE_NUM * (uint32_t)hbond] +
             cData.pKerconst_intra
                 ->reqm_const[atom2_type_vdw_hb + ATYPE_NUM * (uint32_t)hbond]);
        ...
        uint32_t idx =
            atom1_typeid * cData.dockpars.num_of_atypes + atom2_typeid;
        ...
        priv_gradient_per_intracontributor +=
            ((6.0f + 4.0f * (uint32_t)hbond) * B - 12.0f * A) /
            smoothed_distance;
#if defined(DEBUG_ENERGY_KERNEL)
        intraE += A - B;
#endif
      ...
      if (atomic_distance < 20.48f) {
        ...
        /*			float exp_el_DIEL_K = exp_el + DIEL_K;
                                float upper = DIEL_A *
           exp_el_DIEL_K*exp_el_DIEL_K + DIEL_B * exp_el *
           (DIEL_B_TIMES_H_TIMES_K*atomic_distance + exp_el_DIEL_K); float lower
           = atomic_distance * (DIEL_A * exp_el_DIEL_K + DIEL_B * exp_el);
                                lower *= lower;*/
        ...
        priv_gradient_per_intracontributor +=
            -(es_energy / atomic_distance) * diel -
            es_energy * ((2.808f / (dist2 * dist_shift)) +
                         (0.288f / (disth4 * disth_shift))) -
...
#else
    int priv_intra_gradient_x = lrintf(
        fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * subx * grad_div_dist)));
    int priv_intra_gradient_y = lrintf(
        fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * suby * grad_div_dist)));
    int priv_intra_gradient_z = lrintf(
        fminf(MAXTERM, fmaxf(-MAXTERM, TERMSCALE * subz * grad_div_dist)));
#endif
  ...
  float gy = 0.0f;
  float gz = 0.0f;
  for (uint32_t atom_cnt = threadIdx.x; atom_cnt < cData.dockpars.num_of_atoms;
       atom_cnt += blockDim.x) {
    float3 r;
  ...
  if (threadIdx.x == 0) {
    ...
    */
    ...
    float4 quat_torque;
    quat_torque.x =
        torque_rot.x * SIN_HALF_INFINITESIMAL_RADIAN / torque_length;
    quat_torque.y =
        torque_rot.y * SIN_HALF_INFINITESIMAL_RADIAN / torque_length;
    quat_torque.z =
        torque_rot.z * SIN_HALF_INFINITESIMAL_RADIAN / torque_length;
...
// Derived from autodockdev/motion.py/_get_cube3_gradient
    ...
    target_theta =
        PI_TIMES_2 + is_theta_gt_pi * fast_acos(target_q.z / sin_ang);
    target_phi = fmod_pi2(
    ...
    */
    ...
    uint index_theta =
        floor((current_theta - cData.pMem_angle_const[0]) * inv_angle_delta);
    uint index_rotangle =
        floor((current_rotangle - cData.pMem_angle_const[0]) * inv_angle_delta);
    ...
    if ((index_theta <= 0) || (index_theta >= 999)) {
      dependence_on_theta =
          cData.pMem_dependence_on_theta_const[stick_to_bounds(index_theta, 0,
                                                               999)];
    } else {
    ...
    if ((index_rotangle <= 0) || (index_rotangle >= 999)) {
      dependence_on_rotangle =
          cData.pMem_dependence_on_rotangle_const[stick_to_bounds(
              index_rotangle, 0, 999)];
    } else {
    ...
    // gradients)
    gradient_genotype[3] = lrintf(fminf(
        MAXTERM, fmaxf(-MAXTERM, TERMSCALE *
                                     (grad_phi / (dependence_on_theta *
                                                  dependence_on_rotangle)) *
                                     DEG_TO_RAD)));
    gradient_genotype[4] = lrintf(fminf(
        MAXTERM,
        fmaxf(-MAXTERM,
              TERMSCALE * (grad_theta / dependence_on_rotangle) * DEG_TO_RAD)));
    gradient_genotype[5] = lrintf(fminf(
        MAXTERM, fmaxf(-MAXTERM, TERMSCALE * grad_rotangle * DEG_TO_RAD)));
#if defined(PRINT_GRAD_ROTATION_GENES)
    printf("\n%s\n",
           "----------------------------------------------------------");
  ...
  for (uint32_t idx = threadIdx.x;
    ...
    if (rotable_atom_cnt >=
        cData.pMem_num_rotating_atoms_per_rotbond_const[rotbond_id])
    ...
    float l =
        rnorm3df(rotation_unitvec.x, rotation_unitvec.y, rotation_unitvec.z);
    ...
    // Torque of torsions
    uint lig_atom_id =
        cData.pMem_rotbonds_atoms_const[MAX_NUM_OF_ATOMS * rotbond_id +
                                        rotable_atom_cnt];
    float4 torque_tor;
    float3 r, atom_force;
    ...
    float torque_on_axis =
        (rotation_unitvec.x * torque_tor.x + rotation_unitvec.y * torque_tor.y +
    ...
    ATOMICADDI32(
        ...
        lrintf(fminf(MAXTERM,
                     fmaxf(-MAXTERM, TERMSCALE * torque_on_axis *
  ...
  for (uint32_t gene_cnt = threadIdx.x; gene_cnt < cData.dockpars.num_of_genes;
       gene_cnt += blockDim.x) {
    fgradient_genotype[gene_cnt] =
        ONEOVERTERMSCALE * (float)gradient_genotype[gene_cnt];
  }
  __threadfence();
  __syncthreads();
  ...
  for (uint32_t gene_cnt =
           threadIdx.x + 3;  // Only for gene_cnt > 2 means start gene_cnt at 3
       gene_cnt < cData.dockpars.num_of_genes; gene_cnt += blockDim.x) {
    fgradient_genotype[gene_cnt] *= cData.dockpars.grid_spacing *
                                    cData.dockpars.grid_spacing *
