...
__forceinline__ __device__ float fast_acos(float cosine) {
  ...
  float ac = (((fast_acos_o * x4 + fast_acos_a) * x3 + fast_acos_b) * x2 +
              fast_acos_c) *
                 x +
             fast_acos_d + fast_acos_e * sqrt(2.0f - sqrt(2.0f + 2.0f * x)) -
...
__device__ void gpu_calc_energy(float* pGenotype, float& energy, int& run_id,
                                float3* calc_coords, float* pFloatAccumulator)

// The GPU device function calculates the energy of the entity described by
// genotype, dockpars and the liganddata arrays in constant memory and returns it
// in the energy parameter. The parameter run_id has to be equal to the ID of the
// run whose population includes the current entity (which can be determined with
// blockIdx.x), since this determines which reference orientation should be used.
{
  energy = 0.0f;
  ...
  for (uint atom_id = threadIdx.x; atom_id < cData.dockpars.num_of_atoms;
       atom_id += blockDim.x) {
  ...
  for (uint atom_id = threadIdx.x; atom_id < cData.dockpars.num_of_atoms;
    ...
    float* grid_value_000 =
        cData.pMem_fgrids + ((x_low + y_low * g1 + z_low * g2) << 2);
  ...
  {
    // Getting atom IDs
    uint atom1_id = cData.pKerconst_intracontrib
    ...
    float atomic_distance = sqrt(subx * subx + suby * suby + subz * subz) *
...
#if defined(DEBUG_ENERGY_KERNEL)
      intraE +=
          (cData.dockpars.coeff_elec * q1 * q2) /
              (atomic_distance *
               (DIEL_A +
                (DIEL_B / (1.0f + DIEL_K * native_exp(-DIEL_B_TIMES_H *
                                                      atomic_distance))))) +
          ((cData.pKerconst_intra->dspars_S_const[atom1_typeid] +
            cData.dockpars.qasp * fabs(q1)) *
               cData.pKerconst_intra->dspars_V_const[atom2_typeid] +
           (cData.pKerconst_intra->dspars_S_const[atom2_typeid] +
            cData.dockpars.qasp * fabs(q2)) *
               cData.pKerconst_intra->dspars_V_const[atom1_typeid]) *
              cData.dockpars.coeff_desolv *
              exp(-0.03858025f * pow(atomic_distance, 2));
#endif
