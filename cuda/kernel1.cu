...
__global__ void __launch_bounds__(NUM_OF_THREADS_PER_BLOCK,
  ...
  float* pGenotype =
      pMem_conformations_current + blockIdx.x * GENOTYPE_LENGTH_IN_GLOBMEM;
...
void gpu_calc_initpop(uint32_t blocks, uint32_t threadsPerBlock,
                      float* pConformations_current, float* pEnergies_current) {
  gpu_calc_initpop_kernel<<<blocks, threadsPerBlock>>>(pConformations_current,
                                                       pEnergies_current);
