...
#endif
    gpu_perform_LS_kernel(float* pMem_conformations_next,
  ...
  while ((iteration_cnt < cData.dockpars.max_num_of_iters) &&
    ...
    for (uint32_t gene_counter = threadIdx.x;
      ...
      genotype_deviate[gene_counter] =
          rho * (2 * gpu_randf(cData.pMem_prng_states) - 1) *
          (gpu_randf(cData.pMem_prng_states) < gene_scale);
    ...
    // ==================================================================
    gpu_calc_energy(genotype_candidate, candidate_energy, run_id, calc_coords,
                    &sFloatAccumulator);
    // =================================================================
...
void gpu_perform_LS(uint32_t blocks, uint32_t threads,
                    float* pMem_conformations_next,
                    float* pMem_energies_next) {
  // size_t sz_shared = (9 * cpuData.dockpars.num_of_atoms + 5 *
  // cpuData.dockpars.num_of_genes) * sizeof(float);
  size_t sz_shared =
      (3 * cpuData.dockpars.num_of_atoms + 4 * cpuData.dockpars.num_of_genes) *
      sizeof(float);

  gpu_perform_LS_kernel<<<blocks, threads, sz_shared>>>(pMem_conformations_next,
                                                        pMem_energies_next);
