...
__global__ void __launch_bounds__(NUM_OF_THREADS_PER_BLOCK,
    ...
    gpu_gen_and_eval_newpops_kernel(float* pMem_conformations_current,
...
// The GPU global function
{
  ...
  if ((blockIdx.x % cData.dockpars.pop_size) == 0) {
    ...
    // Scan through population (we already picked up a blockDim's worth above so
    // skip)
    for (int i = blockIdx.x + blockDim.x + threadIdx.x;
         i < blockIdx.x + cData.dockpars.pop_size; i += blockDim.x) {
    ...
    for (int i = threadIdx.x; i < cData.dockpars.num_of_genes;
         i += blockDim.x) {
      pMem_conformations_next[dOffset + i] =
          pMem_conformations_current[sOffset + i];
    }
  } else {
    ...
    for (uint32_t gene_counter = threadIdx.x; gene_counter < 10;
    ...
    if (threadIdx.x <
        4)  // it is not ensured that the four candidates will be different...
    {
      parent_candidates[threadIdx.x] =(int)(cData.dockpars.pop_size *
                                            randnums[threadIdx.x]);  // using randnums[0..3]
      candidate_energies[threadIdx.x] =
          pMem_energies_current[run_id * cData.dockpars.pop_size +
                                parent_candidates[threadIdx.x]];
    }
    __threadfence();
    __syncthreads();
    ...
    if (threadIdx.x < 2) {
      ...
      if (candidate_energies[2 * threadIdx.x] <
        ...
        if (/*100.0f**/ randnums[4 + threadIdx.x] <
    ...
    if (/*100.0f**/ randnums[6] < cData.dockpars.crossover_rate)  // Using randnums[6]
    {
      if (threadIdx.x < 2) {
        // Using randnum[7..8]
        covr_point[threadIdx.x] = (int)((cData.dockpars.num_of_genes - 1) *
                                        randnums[7 + threadIdx.x]);
      }
      __threadfence();
      __syncthreads();
        ...
        if (covr_point[0] != covr_point[1]) {
          if ((gene_counter <= covr_point[0]) || (gene_counter > covr_point[1]))
            offspring_genotype[gene_counter] =
                pMem_conformations_current[(run_id * cData.dockpars.pop_size +
                                            parents[0]) *
      ...
      if (/*100.0f**/ gpu_randf(cData.pMem_prng_states) <
        ...
        if (gene_counter < 3) {
          offspring_genotype[gene_counter] +=
              ...
              (2 * gpu_randf(cData.pMem_prng_states) - 1);
        }
    ...
    for (uint32_t gene_counter = threadIdx.x;
         gene_counter < cData.dockpars.num_of_genes;
         gene_counter += blockDim.x) {
      pMem_conformations_next[blockIdx.x * GENOTYPE_LENGTH_IN_GLOBMEM +
                              gene_counter] = offspring_genotype[gene_counter];
    }
  }
#if 0
...
void gpu_gen_and_eval_newpops(uint32_t blocks, uint32_t threadsPerBlock,
                              float* pMem_conformations_current,
                              float* pMem_energies_current,
                              float* pMem_conformations_next,
                              float* pMem_energies_next) {
  gpu_gen_and_eval_newpops_kernel<<<blocks, threadsPerBlock>>>(
      pMem_conformations_current, pMem_energies_current,
