...
__global__ void __launch_bounds__(NUM_OF_THREADS_PER_BLOCK,
                                  1024 / NUM_OF_THREADS_PER_BLOCK)
...
// instead of the first one (with ID 0).
{
  ...
  float3* cartesian_gradient =
      (float3*)(calc_coords + cData.dockpars.num_of_atoms);
#else
  ...
  if (threadIdx.x == 0) {
    ...
    if (entity_id == 0) {
      ...
      if (100.0f * gpu_randf(cData.pMem_prng_states) >
        ...
        entity_id =
            cData.dockpars
                .num_of_lsentities;  // AT - Should this be
  ...
  for (uint32_t i = threadIdx.x; i < cData.dockpars.num_of_genes;
       i += blockDim.x) {
    gradient[i] = 0.0f;
    mt[i] = 0.0f;
    vt[i] = 0.0f;
...
// Printing number of ADADELTA iterations
    ...
    gpu_calc_energrad(
        ...
        // Calculate gradients (forces) for intermolecular energy
        // Derived from autodockdev/maps.py
        cartesian_gradient, gradient, &sFloatAccumulator);
...
// =============================================================
      ...
      printf("%-10s %-10.6f \n", "grids: ", interE);
                        ...
                        for (uint i = 0; i < cData.dockpars.num_of_genes; i++) {
                          if (i == 0) {
                            ...
                            printf("%13s %13s %5s %15s %15s\n", "gene_id",
                                   "gene.value", "|", "gene.grad",
                                   "(autodockdevpy units)");
                          }
                          printf("%13u %13.6f %5s %15.6f %15.6f\n", i,
                                 genotype[i], "|", gradient[i],
                                 (i < 3) ? (gradient[i] / 0.375f)
                        ...
                        for (uint i = 0; i < cData.dockpars.num_of_atoms; i++) {
                          ...
                          printf("%12u %12.6f %12.6f %12.6f\n", i,
                                 calc_coords_x[i], calc_coords_y[i],
                                 calc_coords_z[i]);
                        }
                        printf("\n");
#endif
    ...
    for (int i = threadIdx.x; i < cData.dockpars.num_of_genes;
         i += blockDim.x) {
      if (energy <
      ...
      mt[i] = cData.dockpars.adam_beta1 * mt[i] +
              (1.0f - cData.dockpars.adam_beta1) * gradient[i];
      vt[i] = cData.dockpars.adam_beta2 * vt[i] +
              (1.0f - cData.dockpars.adam_beta2) * gradient[i] * gradient[i];
  ...
  for (uint32_t gene_counter = threadIdx.x + 3;
       gene_counter < cData.dockpars.num_of_genes; gene_counter += blockDim.x) {
  ...
  for (uint gene_counter = threadIdx.x;
       gene_counter < cData.dockpars.num_of_genes; gene_counter += blockDim.x) {
    pMem_conformations_next[gene_counter + offset] =
...
void gpu_gradient_minAdam(uint32_t blocks, uint32_t threads,
                          float* pMem_conformations_next,
                          float* pMem_energies_next) {
  size_t sz_shared =
      (6 * cpuData.dockpars.num_of_atoms + 5 * cpuData.dockpars.num_of_genes) *
      sizeof(float);
  gpu_gradient_minAdam_kernel<<<blocks, threads, sz_shared>>>(
