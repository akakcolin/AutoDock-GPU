...
// Each thread (supposing num_of_runs*pop_size blocks and
// NUM_OF_THREADS_PER_BLOCK threads per block) has its own state which is stored
// in the global memory area pointed by prng_states (thread with ID tx in block
// with ID bx stores its state in prng_states[bx*NUM_OF_THREADS_PER_BLOCK+$ The
// random number generator uses the gcc linear congruential generator constants.
{
  uint state;

  // Current state of the threads own PRNG
  // state = prng_states[get_group_id(0)*NUM_OF_THREADS_PER_BLOCK +
  // get_local_id(0)];
  ...
  // prng_states[get_group_id(0)*NUM_OF_THREADS_PER_BLOCK + get_local_id(0)] =
  // state;
...
void gpu_perform_elitist_selection(
    int dockpars_pop_size, __global float* restrict dockpars_energies_current,
    __global float* restrict dockpars_energies_next,
    __global int* restrict dockpars_evals_of_new_entities,
    int dockpars_num_of_genes,
    __global float* restrict dockpars_conformations_next,
    __global const float* restrict dockpars_conformations_current,
    __local float* best_energies, __local int* best_IDs, __local int* best_ID)
...
// conformations_next and energies_next.
{
  ...
  for (entity_counter = NUM_OF_THREADS_PER_BLOCK + tidx;
    ...
    if (dockpars_energies_current[get_group_id(0) + entity_counter] <
        best_energies[tidx]) {
      best_energies[tidx] =
          dockpars_energies_current[get_group_id(0) + entity_counter];
  ...
  if (tidx == 0) {
    ...
    for (entity_counter = 1; entity_counter < NUM_OF_THREADS_PER_BLOCK;
         entity_counter++) {
      if ((best_energies[entity_counter] < best_energy) &&
          (entity_counter < dockpars_pop_size)) {
    ...
    // Zero (0) evals were performed for entity selected with elitism (since it
    // was copied only)
  ...
  for (gene_counter = tidx; gene_counter < dockpars_num_of_genes;
    ...
    dockpars_conformations_next[GENOTYPE_LENGTH_IN_GLOBMEM * get_group_id(0) +
                                gene_counter] =
        dockpars_conformations_current[GENOTYPE_LENGTH_IN_GLOBMEM *
                                           get_group_id(0) +
                                       GENOTYPE_LENGTH_IN_GLOBMEM * best_ID[0] +
                                       gene_counter];
  }
}
