...
#define TERMBITS 10
#define MAXTERM \
  ((float)(1 << (31 - TERMBITS - 8)))        // 2^(31 - 10 - 8) = 2^13 = 8192
...
// The latter can be distinguised this way: they are place within lines without
// indentation.

void gpu_calc_energrad(
    int dockpars_rotbondlist_length, int dockpars_num_of_atoms,
    int dockpars_gridsize_x, int dockpars_gridsize_y, int dockpars_gridsize_z,
    // g1 = gridsize_x
    uint dockpars_gridsize_x_times_y,          // g2 = gridsize_x * gridsize_y
    uint dockpars_gridsize_x_times_y_times_z,  // g3 = gridsize_x * gridsize_y *
                                               // gridsize_z
    __global const float* restrict
        dockpars_fgrids,  // This is too large to be allocated in __constant
    int dockpars_num_of_atypes, int dockpars_num_of_map_atypes,
    ...
    __local float* genotype, __local float* energy, __local int* run_id,

    __local float4* calc_coords, __local float* partial_energies,
#if defined(DEBUG_ENERGY_KERNEL)
    __local float* partial_interE, __local float* partial_intraE,
#endif
    ...
    __constant int* rotbonds_const, __global const int* rotbonds_atoms_const,
    ...
    __local float* gradient_x, __local float* gradient_y,
    __local float* gradient_z,
#else
    __local int* gradient_x, __local int* gradient_y, __local int* gradient_z,
#endif
    __local float* accumulator_x, __local float* accumulator_y,
    __local float* accumulator_z, __local float* gradient_genotype) {
  ...
  for (uint atom_id = tidx;
       atom_id <
       MAX_NUM_OF_ATOMS;  // makes sure that gradient sum reductions give
                          // correct results if dockpars_num_atoms <
                          // NUM_OF_THREADS_PER_BLOCK
       atom_id += NUM_OF_THREADS_PER_BLOCK) {
    // Initialize coordinates
    calc_coords[atom_id] =
        (float4)(kerconst_conform->ref_coords_const[3 * atom_id],
  ...
  for (uint gene_cnt = tidx; gene_cnt < dockpars_num_of_genes;
  ...
  for (uint rotation_counter = tidx;
    ...
    int rotation_list_element =
        kerconst_rotlist->rotlist_const[rotation_counter];

    if ((rotation_list_element & RLIST_DUMMY_MASK) ==
        0)  // If not dummy rotation
    {
      ...
      if ((rotation_list_element & RLIST_GENROT_MASK) ==
          0)  // If rotating around rotatable bond
      {
        uint rotbond_id =
            (rotation_list_element & RLIST_RBONDID_MASK) >> RLIST_RBONDID_SHIFT;

        float rotation_angle = genotype[6 + rotbond_id] * DEG_TO_RAD * 0.5f;
        float s = native_sin(rotation_angle);
        rotation_unitvec = (float4)(
            s * kerconst_conform->rotbonds_unit_vectors_const[3 * rotbond_id],
            s * kerconst_conform
                    ->rotbonds_unit_vectors_const[3 * rotbond_id + 1],
            s * kerconst_conform
                    ->rotbonds_unit_vectors_const[3 * rotbond_id + 2],
            native_cos(rotation_angle));
        rotation_movingvec = (float4)(
            kerconst_conform->rotbonds_moving_vectors_const[3 * rotbond_id],
            kerconst_conform->rotbonds_moving_vectors_const[3 * rotbond_id + 1],
            kerconst_conform->rotbonds_moving_vectors_const[3 * rotbond_id + 2],
            0);
              ...
              // (multiplying the quaternions)
      {
        ...
        uint rid4 = (*run_id) << 2;
        quatrot_left = quaternion_multiply(
            quatrot_left,
      ...
      calc_coords[atom_id] =
          quaternion_rotate(atom_to_rotate, quatrot_left) + rotation_movingvec;
  ...
  float weights[8];
  float cube[8];
  for (uint atom_id = tidx; atom_id < dockpars_num_of_atoms;
       atom_id += NUM_OF_THREADS_PER_BLOCK) {
    ...
    if ((x < 0) || (y < 0) || (z < 0) || (x >= dockpars_gridsize_x - 1) ||
        (y >= dockpars_gridsize_y - 1) || (z >= dockpars_gridsize_z - 1)) {
...
#ifdef FLOAT_GRADIENTS
      gradient_x[atom_id] +=
          native_divide(TERMSCALE * 42.0f * x, dockpars_grid_spacing);
      gradient_y[atom_id] +=
          native_divide(TERMSCALE * 42.0f * y, dockpars_grid_spacing);
      gradient_z[atom_id] +=
          native_divide(TERMSCALE * 42.0f * z, dockpars_grid_spacing);
#else
      gradient_x[atom_id] += convert_int_rte(
          native_divide(TERMSCALE * 42.0f * x, dockpars_grid_spacing));
      gradient_y[atom_id] += convert_int_rte(
          native_divide(TERMSCALE * 42.0f * y, dockpars_grid_spacing));
      gradient_z[atom_id] += convert_int_rte(
          native_divide(TERMSCALE * 42.0f * z, dockpars_grid_spacing));
    ...
    // Grid value at 000
    __global const float* grid_value_000 =
        dockpars_fgrids + ((ulong)(x_low + y_low * g1 + z_low * g2) << 2);
    ...
    partial_energies[tidx] += cube[0] * weights[0] + cube[1] * weights[1] +
                              cube[2] * weights[2] + cube[3] * weights[3] +
                              cube[4] * weights[4] + cube[5] * weights[5] +
                              cube[6] * weights[6] + cube[7] * weights[7];
#if defined(DEBUG_ENERGY_KERNEL)
    partial_interE[tidx] += cube[0] * weights[0] + cube[1] * weights[1] +
                            cube[2] * weights[2] + cube[3] * weights[3] +
                            cube[4] * weights[4] + cube[5] * weights[5] +
                            cube[6] * weights[6] + cube[7] * weights[7];
#endif
    ...
    */
    ...
    /*		x10 = cube [idx_100] - cube [idx_000]; // z = 0
                    ...
                    vx_z0 = omdy * (cube [idx_100] - cube [idx_000]) + dy *
       (cube [idx_110] - cube [idx_010]);     // z = 0 vx_z1 = omdy * (cube
       [idx_101] - cube [idx_001]) + dy * (cube [idx_111] - cube [idx_011]);
       // z = 1 */

    // AT - all in one go with no intermediate variables (following calcs are
    // similar) Vector in x-direction
    float gx = native_divide(omdz * (omdy * (cube[idx_100] - cube[idx_000]) +
                                     dy * (cube[idx_110] - cube[idx_010])) +
                                 dz * (omdy * (cube[idx_101] - cube[idx_001]) +
                                       dy * (cube[idx_111] - cube[idx_011])),
                             dockpars_grid_spacing);
    // Vector in y-direction
    float gy = native_divide(omdz * (omdx * (cube[idx_010] - cube[idx_000]) +
                                     dx * (cube[idx_110] - cube[idx_100])) +
                                 dz * (omdx * (cube[idx_011] - cube[idx_001]) +
                                       dx * (cube[idx_111] - cube[idx_101])),
                             dockpars_grid_spacing);
    // Vectors in z-direction
    float gz = native_divide(omdy * (omdx * (cube[idx_001] - cube[idx_000]) +
                                     dx * (cube[idx_101] - cube[idx_100])) +
                                 dy * (omdx * (cube[idx_011] - cube[idx_010]) +
                                       dx * (cube[idx_111] - cube[idx_110])),
                             dockpars_grid_spacing);
    // -------------------------------------------------------------------
    // Calculating gradients (forces) corresponding to
    ...
    // Calculating affinity energy
    partial_energies[tidx] += q * (cube[0] * weights[0] + cube[1] * weights[1] +
                                   cube[2] * weights[2] + cube[3] * weights[3] +
                                   cube[4] * weights[4] + cube[5] * weights[5] +
                                   cube[6] * weights[6] + cube[7] * weights[7]);
#if defined(DEBUG_ENERGY_KERNEL)
    partial_interE[tidx] += q * (cube[0] * weights[0] + cube[1] * weights[1] +
                                 cube[2] * weights[2] + cube[3] * weights[3] +
                                 cube[4] * weights[4] + cube[5] * weights[5] +
                                 cube[6] * weights[6] + cube[7] * weights[7]);
#endif
    ...
    // Vector in x-direction
    gx += qg * (omdz * (omdy * (cube[idx_100] - cube[idx_000]) +
                        dy * (cube[idx_110] - cube[idx_010])) +
                dz * (omdy * (cube[idx_101] - cube[idx_001]) +
                      dy * (cube[idx_111] - cube[idx_011])));
    // Vector in y-direction
    gy += qg * (omdz * (omdx * (cube[idx_010] - cube[idx_000]) +
                        dx * (cube[idx_110] - cube[idx_100])) +
                dz * (omdx * (cube[idx_011] - cube[idx_001]) +
                      dx * (cube[idx_111] - cube[idx_101])));
    // Vectors in z-direction
    gz += qg * (omdy * (omdx * (cube[idx_001] - cube[idx_000]) +
                        dx * (cube[idx_101] - cube[idx_100])) +
                dy * (omdx * (cube[idx_011] - cube[idx_010]) +
                      dx * (cube[idx_111] - cube[idx_110])));
    // -------------------------------------------------------------------
    ...
    q = fabs(q);
    // Capturing desolvation values (atom_typeid+1 compared to above => mul_tmp
    // + g3*4)
    mul_tmp += g3 << 2;
    cube[0] = *(grid_value_000 + mul_tmp + 0);
    cube[1] = *(grid_value_000 + mul_tmp + 1);
...
#else
    gradient_x[atom_id] +=
        convert_int_rte(fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * gx)));
    gradient_y[atom_id] +=
        convert_int_rte(fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * gy)));
    gradient_z[atom_id] +=
        convert_int_rte(fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * gz)));
#endif
  ...
  for (uint contributor_counter = tidx;
    ...
    uint atom1_type_vdw_hb =
        kerconst_intra->atom1_types_reqm_const[atom1_typeid];
    uint atom2_type_vdw_hb =
        kerconst_intra->atom2_types_reqm_const[atom2_typeid];
    ...
    float vbond = G * (float)(((atom1_type_vdw_hb == ATYPE_CG_IDX) &&
    ...
    if (atomic_distance < 8.0f) {
      ...
      //  hbond
      float opt_distance =
          (kerconst_intra->reqm_const[atom1_type_vdw_hb + ATYPE_NUM * hbond] +
           kerconst_intra->reqm_const[atom2_type_vdw_hb + ATYPE_NUM * hbond]);
      ...
      if (fabs(opt_dist_delta) >= delta_distance) {
        smoothed_distance =
            atomic_distance + copysign(delta_distance, opt_dist_delta);
      } else
        smoothed_distance = opt_distance;
      // Calculating van der Waals / hydrogen bond term
      uint idx = atom1_typeid * dockpars_num_of_atypes + atom2_typeid;
      float nvbond = 1.0 - vbond;
      float A = nvbond * native_divide(kerconst_intra->VWpars_AC_const[idx],
                                       native_powr(smoothed_distance, 12));
      float B =
          nvbond * native_divide(kerconst_intra->VWpars_BD_const[idx],
                                 native_powr(smoothed_distance, 6 + 4 * hbond));
      partial_energies[tidx] += A - B;
      priv_gradient_per_intracontributor += native_divide(
          (6.0f + 4.0f * hbond) * B - 12.0f * A, smoothed_distance);
#if defined(DEBUG_ENERGY_KERNEL)
    ...
    if (atomic_distance < 20.48f) {
      ...
      //			float exp_el =
      //native_exp(DIEL_B_TIMES_H*atomic_distance);
      float dist2 = atomic_distance * atomic_distance;
      // Calculating desolvation term
      // 1/25.92 = 0.038580246913580245
      float desolv_energy =
          ((kerconst_intra->dspars_S_const[atom1_typeid] +
            dockpars_qasp * fabs(q1)) *
               kerconst_intra->dspars_V_const[atom2_typeid] +
           (kerconst_intra->dspars_S_const[atom2_typeid] +
            dockpars_qasp * fabs(q2)) *
               kerconst_intra->dspars_V_const[atom1_typeid]) *
          native_divide(
              dockpars_coeff_desolv *
                  (12.96f - 0.1063f * dist2 * (1.0f - 0.001947f * dist2)),
              (12.96f +
               dist2 * (0.4137f + dist2 * (0.00357f + 0.000112f * dist2))));
      //						dockpars_coeff_desolv*native_exp(-0.03858025f*atomic_distance*atomic_distance);
      // Calculating electrostatic term
      /*			partial_energies[tidx] += native_divide (
                                                                        dockpars_coeff_elec
         * q1 * q2, atomic_distance * (DIEL_A + native_divide(DIEL_B,(1.0f +
         native_divide(DIEL_K,exp_el)))) ) + desolv_energy;*/
      ...
      disth4 *= disth4;
      float diel = native_divide(1.404f, dist2) +
                   native_divide(0.072f, disth4) + 0.00831f;
      float es_energy =
          native_divide(dockpars_coeff_elec * q1 * q2, atomic_distance);
      ...
      /*			float exp_el_DIEL_K = exp_el + DIEL_K;
         ...
         + DIEL_B * exp_el * (DIEL_B_TIMES_H_TIMES_K*atomic_distance +
         exp_el_DIEL_K); float lower = atomic_distance * (DIEL_A * exp_el_DIEL_K
         + DIEL_B * exp_el); lower *= lower;*/

      //			priv_gradient_per_intracontributor +=
      //-dockpars_coeff_elec * q1 * q2 * native_divide (upper, lower) -
      //								0.0771605f
      //* atomic_distance * desolv_energy;
      priv_gradient_per_intracontributor +=
          -native_divide(es_energy, atomic_distance) * diel -
          es_energy * (native_divide(2.808f, dist2 * dist_shift) +
                       native_divide(0.288f, disth4 * disth_shift)) -
          0.0771605f * atomic_distance *
              desolv_energy;  // 1/3.6^2 = 1/12.96 = 0.0771605
    ...
    float grad_div_dist =
        native_divide(-priv_gradient_per_intracontributor, dist);
#ifdef FLOAT_GRADIENTS
...
#else
    int priv_intra_gradient_x = convert_int_rte(
        fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * subx * grad_div_dist)));
    int priv_intra_gradient_y = convert_int_rte(
        fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * suby * grad_div_dist)));
    int priv_intra_gradient_z = convert_int_rte(
        fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * subz * grad_div_dist)));
#endif
  ...
  for (uint atom_cnt = tidx; atom_cnt < dockpars_num_of_atoms;
  ...
  if (tidx == 0) {
    *energy = partial_energies[0];
    ...
    i_gradient_genotype[0] = convert_int_rte(fmin(
        MAXTERM,
        fmax(-MAXTERM, TERMSCALE * accumulator_x[0] * dockpars_grid_spacing)));
    i_gradient_genotype[1] = convert_int_rte(fmin(
        MAXTERM,
        fmax(-MAXTERM, TERMSCALE * accumulator_y[0] * dockpars_grid_spacing)));
    i_gradient_genotype[2] = convert_int_rte(fmin(
        MAXTERM,
        fmax(-MAXTERM, TERMSCALE * accumulator_z[0] * dockpars_grid_spacing)));
  ...
  for (uint atom_cnt = tidx; atom_cnt < dockpars_num_of_atoms;
       atom_cnt += NUM_OF_THREADS_PER_BLOCK) {
    float4 r =
        (calc_coords[atom_cnt] - genrot_movingvec) * dockpars_grid_spacing;
  ...
  // do a reduction over the total gradient containing prepared
  // "gradient_intra_*" values
  for (uint off = NUM_OF_THREADS_PER_BLOCK >> 1; off > 0; off >>= 1) {
    barrier(CLK_LOCAL_MEM_FENCE);
    if (tidx < off) {
      accumulator_x[tidx] += accumulator_x[tidx + off];
  ...
  if (tidx == 0) {
    ...
    */
    ...
    float4 quat_torque =
        torque_rot *
        native_divide(SIN_HALF_INFINITESIMAL_RADIAN, torque_length);
    ...
    printf("%-20s %-10.6f %-10.6f %-10.6f %-10.6f\n",
           "quat_torque (w,x,y,z): ", quat_torque.w, quat_torque.x,
           quat_torque.y, quat_torque.z);
#endif
...
// Derived from autodockdev/motion.py/_get_cube3_gradient
    ...
    printf("%-30s %-10.6f %-10.6f %-10.6f %-10.6f\n",
           "current_q (w,x,y,z): ", genrot_unitvec.w, genrot_unitvec.x,
           genrot_unitvec.y, genrot_unitvec.z);
#endif
    ...
    printf("\n%s\n",
           "----------------------------------------------------------");
    printf("%-30s %-10.6f %-10.6f %-10.6f %-10.6f\n",
           "target_q (w,x,y,z): ", target_q.w, target_q.x, target_q.y,
           target_q.z);
#endif
    ...
    // In our terms means quaternion_to_oclacube(target_q{w|x|y|z},
    // theta_larger_than_pi)
    target_rotangle = 2.0f * fast_acos(target_q.w);  // = 2.0f * ang;
    float sin_ang =
        native_sqrt(1.0f - target_q.w * target_q.w);  // = native_sin(ang);

    target_theta = PI_TIMES_2 + is_theta_gt_pi * fast_acos(native_divide(
                                                     target_q.z, sin_ang));
    target_phi = fmod_pi2(
        (atan2(is_theta_gt_pi * target_q.y, is_theta_gt_pi * target_q.x) +
         PI_TIMES_2));
    ...
    printf("%-30s %-10.6f %-10.6f %-10.6f\n",
           "target_axisangle (1,2,3): ", target_phi, target_theta,
           target_rotangle);
#endif
    ...
    // float shoemake_scaling = native_divide(torque_length,
    // INFINITESIMAL_RADIAN/*infinitesimal_radian*/);
    ...
    grad_phi = orientation_scaling *
               (fmod_pi2(target_phi - current_phi + PI_FLOAT) - PI_FLOAT);
    grad_theta = orientation_scaling *
                 (fmod_pi2(target_theta - current_theta + PI_FLOAT) - PI_FLOAT);
    grad_rotangle =
        orientation_scaling *
        (fmod_pi2(target_rotangle - current_rotangle + PI_FLOAT) - PI_FLOAT);
    ...
    */
    ...
    // uint index_theta    = floor(native_divide(current_theta    -
    // angle_const[0], angle_delta)); uint index_rotangle =
    // floor(native_divide(current_rotangle - angle_const[0], angle_delta));
    uint index_theta =
        floor((current_theta - angle_const[0]) * inv_angle_delta);
    uint index_rotangle =
        floor((current_rotangle - angle_const[0]) * inv_angle_delta);
    ...
    if ((index_theta <= 0) || (index_theta >= 999)) {
      dependence_on_theta =
          dependence_on_theta_const[stick_to_bounds(index_theta, 0, 999)];
    } else {
      ...
      dependence_on_theta =
          (Y0 * (X1 - current_theta) + Y1 * (current_theta - X0)) *
          inv_angle_delta;
    }
    ...
    if ((index_rotangle <= 0) || (index_rotangle >= 999)) {
      dependence_on_rotangle =
          dependence_on_rotangle_const[stick_to_bounds(index_rotangle, 0, 999)];
    } else {
      ...
      dependence_on_rotangle =
          (Y0 * (X1 - current_rotangle) + Y1 * (current_rotangle - X0)) *
          inv_angle_delta;
    }
    ...
    // Multiplicating by DEG_TO_RAD is to make it uniform to DEG (see torsion
    // gradients)
    i_gradient_genotype[3] = convert_int_rte(fmin(
        MAXTERM,
        fmax(-MAXTERM, TERMSCALE *
                           native_divide(grad_phi, (dependence_on_theta *
                                                    dependence_on_rotangle)) *
                           DEG_TO_RAD)));
    i_gradient_genotype[4] = convert_int_rte(fmin(
        MAXTERM,
        fmax(-MAXTERM, TERMSCALE *
                           native_divide(grad_theta, dependence_on_rotangle) *
                           DEG_TO_RAD)));
    i_gradient_genotype[5] = convert_int_rte(
        fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * grad_rotangle * DEG_TO_RAD)));
#if defined(PRINT_GRAD_ROTATION_GENES)
    printf("\n%s\n",
           "----------------------------------------------------------");
    ...
    printf("%-13s %-13s %-13s \n", "grad_phi", "grad_theta", "grad_rotangle");
    printf("%-13.6f %-13.6f %-13.6f\n", gradient_genotype[3],
           gradient_genotype[4], gradient_genotype[5]);
#endif
  }
  ...
  for (uint idx = tidx; idx < num_torsion_genes * dockpars_num_of_atoms;
       idx += NUM_OF_THREADS_PER_BLOCK) {
    uint rotable_atom_cnt = idx / num_torsion_genes;
    uint rotbond_id =
        idx - rotable_atom_cnt *
                  num_torsion_genes;  // this is a bit cheaper than % (modulo)
    ...
    float4 rotation_unitvec =
        fast_normalize(calc_coords[atom2_id] - atomRef_coords);

    // Torque of torsions
    uint lig_atom_id =
        rotbonds_atoms_const[MAX_NUM_OF_ATOMS * rotbond_id + rotable_atom_cnt];
    ...
    float torque_on_axis =
        dot(rotation_unitvec, torque_tor) *
        dockpars_grid_spacing;  // it is cheaper to do a scalar multiplication
                                // than a vector one

    // Assignment of gene-based gradient
    // - this works because a * (a_1 + a_2 + ... + a_n) = a*a_1 + a*a_2 + ... +
    // a*a_n
    atomic_add(
        &i_gradient_genotype[rotbond_id + 6],
        convert_int_rte(
            fmin(MAXTERM, fmax(-MAXTERM, TERMSCALE * torque_on_axis *
  ...
  for (uint gene_cnt = tidx; gene_cnt < dockpars_num_of_genes;
    ...
    gradient_genotype[gene_cnt] =
        ONEOVERTERMSCALE * (float)i_gradient_genotype[gene_cnt];
  }
  barrier(CLK_LOCAL_MEM_FENCE);

#if defined(CONVERT_INTO_ANGSTROM_RADIAN)
  for (uint gene_cnt =
           tidx + 3;  // Only for gene_cnt > 2 means start gene_cnt at 3
       gene_cnt < dockpars_num_of_genes; gene_cnt += NUM_OF_THREADS_PER_BLOCK) {
    gradient_genotype[gene_cnt] *= dockpars_grid_spacing *
                                   dockpars_grid_spacing *
