...
typedef struct {
  float reqm_const[2 * ATYPE_NUM];  // 1st ATYPE_NUM entries = vdW, 2nd
                                    // ATYPE_NUM entries = hbond
...
inline float fmod_pi2(float x) { return x - (int)(invpi2 * x) * PI_TIMES_2; }
...
inline float fast_acos(float cosine) {
  ...
  float x3 = x2 * x;
  float x4 = x3 * x;
  float ac = (((fast_acos_o * x4 + fast_acos_a) * x3 + fast_acos_b) * x2 +
              fast_acos_c) *
                 x +
             fast_acos_d +
             fast_acos_e * native_sqrt(2.0f - native_sqrt(2.0f + 2.0f * x)) -
             fast_acos_f * native_sqrt(2.0f - 2.0f * x);
...
// The GPU device function calculates the energy of the entity described by
// genotype, dockpars and the liganddata arrays in constant memory and returns
// it in the energy parameter. The parameter run_id has to be equal to the ID of
// the run whose population includes the current entity (which can be determined
// with blockIdx.x), since this determines which reference orientation should be
// used.
{
  ...
  for (uint atom_id = tidx; atom_id < dockpars_num_of_atoms;
...
#endif
      continue;  // get on with loop as our work here is done (we crashed into
                 // the walls)
    }
    // Getting coordinates
    ...
    __global const float* grid_value_000 =
        dockpars_fgrids + ((x_low + y_low * g1 + z_low * g2) << 2);
    ulong mul_tmp = atom_typeid * g3 << 2;
    // Calculating affinity energy
    partial_energies[tidx] +=
        TRILININTERPOL((grid_value_000 + mul_tmp), weights);
    ...
    partial_energies[tidx] +=
        q * TRILININTERPOL((grid_value_000 + mul_tmp), weights);
    ...
    partial_energies[tidx] +=
        fabs(q) * TRILININTERPOL((grid_value_000 + mul_tmp), weights);
  ...
  for (uint off = NUM_OF_THREADS_PER_BLOCK >> 1; off > 0; off >>= 1) {
    barrier(CLK_LOCAL_MEM_FENCE);
    if (tidx < off) partial_intraE[tidx] += partial_intraE[tidx + off];
  }
#endif
...
if (tidx == 0) {
...
#endif
  {
    ...
    float atomic_distance =
        native_sqrt(subx * subx + suby * suby + subz * subz) *
    ...
    if (atomic_distance < 8.0f) {
      ...
      partial_energies[tidx] +=
          native_divide(kerconst_intra->VWpars_AC_const[idx],
                        native_powr(smoothed_distance, 12)) -
          native_divide(kerconst_intra->VWpars_BD_const[idx],
               ...
               dist2 *
                   (0.4137f +
                    dist2 *
                        (0.00357f +
                         0.000112f *
                             dist2)))  // *native_exp(-0.03858025f*atomic_distance*atomic_distance);
          );
          ...
          native_divide(
              ...
              atomic_distance *
                  (DIEL_A +
                   native_divide(
                       DIEL_B,
                       (1.0f + DIEL_K * native_exp(-DIEL_B_TIMES_H *
                                                   atomic_distance))))) +
          ((kerconst_intra->dspars_S_const[atom1_typeid] +
            dockpars_qasp * fabs(q1)) *
               kerconst_intra->dspars_V_const[atom2_typeid] +
           (kerconst_intra->dspars_S_const[atom2_typeid] +
            dockpars_qasp * fabs(q2)) *
               kerconst_intra->dspars_V_const[atom1_typeid]) *
              dockpars_coeff_desolv *
              native_exp(-0.03858025f * native_powr(atomic_distance, 2));
#endif
  ...
  if (tidx == 0) *energy = partial_energies[0];
}
