...
// The GPU device function calculates the energy's gradient (forces or
// derivatives) of the entity described by genotype, dockpars and the
// ligand-data arrays in constant memory and returns it in the
// "gradient_genotype" parameter. The parameter "run_id" has to be equal to the
// ID of the run whose population includes the current entity (which can be
// determined with get_group_id(0)), since this determines which reference
...
void gpu_calc_gradient(
    ...
    ,
    int dockpars_num_of_genes, __local float* gradient_inter_x,
    __local float* gradient_inter_y, __local float* gradient_inter_z,
    __local float* gradient_intra_x, __local float* gradient_intra_y,
    __local float* gradient_intra_z, __local float* gradient_genotype) {
  ...
  for (uint rotation_counter = tidx;
        ...
        0)  // If not dummy rotation
    {
      ...
      if ((rotation_list_element & RLIST_FIRSTROT_MASK) !=
          0)  // If first rotation of this atom
      {
      ...
      } else  // If rotating around rotatable bond
      {
        ...
        rotation_unitvec[0] =
            kerconst_conform->rotbonds_unit_vectors_const[3 * rotbond_id];
        rotation_unitvec[1] =
            kerconst_conform->rotbonds_unit_vectors_const[3 * rotbond_id + 1];
        rotation_unitvec[2] =
            kerconst_conform->rotbonds_unit_vectors_const[3 * rotbond_id + 2];
              ...
              // (multiplying the quaternions)
      {
        ...
        quatrot_left_q =
            quatrot_temp_q *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 3] -
            quatrot_temp_x *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 0] -
            quatrot_temp_y *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 1] -
            quatrot_temp_z *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 2];
        quatrot_left_x =
            quatrot_temp_q *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 0] +
            kerconst_conform->ref_orientation_quats_const[4 * (*run_id) + 3] *
                quatrot_temp_x +
            quatrot_temp_y *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 2] -
            kerconst_conform->ref_orientation_quats_const[4 * (*run_id) + 1] *
                quatrot_temp_z;
        quatrot_left_y =
            quatrot_temp_q *
                kerconst_conform
            ...
            kerconst_conform->ref_orientation_quats_const[4 * (*run_id) + 3] *
                quatrot_temp_y +
            kerconst_conform->ref_orientation_quats_const[4 * (*run_id) + 0] *
                quatrot_temp_z -
            quatrot_temp_x *
                kerconst_conform
                    ->ref_orientation_quats_const[4 * (*run_id) + 2];
      ...
      quatrot_temp_q = 0 - quatrot_left_x * atom_to_rotate[0] -
  ...
  for (uint contributor_counter = tidx;
    ...
    */
    ...
    if (kerconst_intracontrib
            ->intraE_contributors_const[3 * contributor_counter + 2] ==
        1)  // H-bond
    {
      opt_distance = kerconst_intra->reqm_const[atom1_type_vdw_hb + ATYPE_NUM] +
                     kerconst_intra->reqm_const[atom2_type_vdw_hb + ATYPE_NUM];
    } else  // van der Waals
    {
      opt_distance = (kerconst_intra->reqm_const[atom1_type_vdw_hb] +
                      kerconst_intra->reqm_const[atom2_type_vdw_hb]);
    }
    ...
    if (atomic_distance < 8.0f) {
      ...
      priv_gradient_per_intracontributor += native_divide(
          -12 * kerconst_intra
                    ->VWpars_AC_const[atom1_typeid * dockpars_num_of_atypes +
                                      atom2_typeid],
      ...
      if (kerconst_intracontrib
        ...
        priv_gradient_per_intracontributor += native_divide(
            10 * kerconst_intra
                     ->VWpars_BD_const[atom1_typeid * dockpars_num_of_atypes +
                                       atom2_typeid],
    ...
    if (atomic_distance < 20.48f) {
      ...
      float upper =
          DIEL_A *
              native_powr(native_exp(DIEL_B_TIMES_H * atomic_distance) + DIEL_K,
                          2) +
          (DIEL_B)*native_exp(DIEL_B_TIMES_H * atomic_distance) *
              (DIEL_B_TIMES_H_TIMES_K * atomic_distance +
               native_exp(DIEL_B_TIMES_H * atomic_distance) + DIEL_K);

      float lower =
          native_powr(atomic_distance, 2) *
          native_powr(
              DIEL_A * (native_exp(DIEL_B_TIMES_H * atomic_distance) + DIEL_K) +
                  DIEL_B * native_exp(DIEL_B_TIMES_H * atomic_distance),
              2);
      ...
      priv_gradient_per_intracontributor +=
          ((kerconst_intra->dspars_S_const[atom1_typeid] +
            dockpars_qasp *
                fabs(kerconst_interintra->atom_charges_const[atom1_id])) *
               kerconst_intra->dspars_V_const[atom2_typeid] +
           (kerconst_intra->dspars_S_const[atom2_typeid] +
            dockpars_qasp *
                fabs(kerconst_interintra->atom_charges_const[atom2_id])) *
               kerconst_intra->dspars_V_const[atom1_typeid]) *
          dockpars_coeff_desolv * /*-0.07716049382716049*/ -0.077160f *
          atomic_distance *
          native_exp(/*-0.038580246913580245*/ -0.038580f *
                     native_powr(atomic_distance, 2));
    ...
    float priv_intra_gradient_x =
        priv_gradient_per_intracontributor * subx_div_dist;
    float priv_intra_gradient_y =
        priv_gradient_per_intracontributor * suby_div_dist;
    float priv_intra_gradient_z =
  ...
  for (uint atom_cnt = tidx; atom_cnt < dockpars_num_of_atoms;
    ...
    gradient_inter_x[atom_cnt] =
        native_divide(gradient_inter_x[atom_cnt], dockpars_grid_spacing);
    gradient_inter_y[atom_cnt] =
        native_divide(gradient_inter_y[atom_cnt], dockpars_grid_spacing);
    gradient_inter_z[atom_cnt] =
        native_divide(gradient_inter_z[atom_cnt], dockpars_grid_spacing);
    ...
    if (atom_cnt == 0) {
      ...
      printf("%10s %13s %13s %13s %5s %13s %13s %13s\n", "atom_id",
             "grad_intER.x", "grad_intER.y", "grad_intER.z", "|",
             "grad_intRA.x", "grad_intRA.y", "grad_intRA.z");
    }
    printf("%10u %13.6f %13.6f %13.6f %5s %13.6f %13.6f %13.6f\n", atom_cnt,
           gradient_inter_x[atom_cnt], gradient_inter_y[atom_cnt],
           gradient_inter_z[atom_cnt], "|", gradient_intra_x[atom_cnt],
           gradient_intra_y[atom_cnt], gradient_intra_z[atom_cnt]);
#endif
    ...
    // printf("%-15s %-5u %-10.8f %-10.8f %-10.8f\n", "grad_grid", atom_cnt,
    // gradient_inter_x[atom_cnt], gradient_inter_y[atom_cnt],
    // gradient_inter_z[atom_cnt]); printf("%-15s %-5u %-10.8f %-10.8f
    // %-10.8f\n", "grad_intra", atom_cnt, gradient_intra_x[atom_cnt],
    // gradient_intra_y[atom_cnt], gradient_intra_z[atom_cnt]); printf("%-15s
    // %-5u %-10.8f %-10.8f %-10.8f\n", "calc_coords", atom_cnt,
    // calc_coords_x[atom_cnt], calc_coords_y[atom_cnt],
    ...
    printf("%10u %13.6f %13.6f %13.6f \n", atom_cnt, gradient_inter_x[atom_cnt],
           gradient_inter_y[atom_cnt], gradient_inter_z[atom_cnt]);
#endif
  }
  ...
  // ------------------------------------------
  if (tidx == 0) {
    for (uint lig_atom_id = 0; lig_atom_id < dockpars_num_of_atoms;
         lig_atom_id++) {
      // Re-using "gradient_inter_*" for total gradient (inter+intra)
      gradient_genotype[0] +=
          gradient_inter_x[lig_atom_id];  // gradient for gene 0: gene x
      gradient_genotype[1] +=
          gradient_inter_y[lig_atom_id];  // gradient for gene 1: gene y
      gradient_genotype[2] +=
          gradient_inter_z[lig_atom_id];  // gradient for gene 2: gene z
    }
    ...
    for (uint lig_atom_id = 0; lig_atom_id < dockpars_num_of_atoms;
      ...
      if (lig_atom_id == 0) {
        ...
        printf("%10s %10s %10s %10s %5s %12s %12s %12s %5s %11s %11s %11s\n",
               "atom_id", "r.x", "r.y", "r.z", "|", "force.x", "force.y",
               "force.z", "|", "torque.x", "torque.y", "torque.z");
      }
      printf(
          "%10u %10.6f %10.6f %10.6f %5s %12.6f %12.6f %12.6f %5s %12.6f "
          "%12.6f %12.6f\n",
...
// lig_atom_id, r.x, r.y, r.z, force.x, force.y, force.z);
    ...
    */
    ...
    quat_torque.x =
        fast_normalize(torque_rot).x * SIN_HALF_INFINITESIMAL_RADIAN;
    quat_torque.y =
        fast_normalize(torque_rot).y * SIN_HALF_INFINITESIMAL_RADIAN;
    quat_torque.z =
        fast_normalize(torque_rot).z * SIN_HALF_INFINITESIMAL_RADIAN;
    ...
    current_rotangle =
        genotype[5];  // rotangle (in sexagesimal (DEG) unbounded)

    map_priv_angle(&current_phi);  // phi      (in DEG bounded within [0, 360])
    map_priv_angle(
        &current_theta);  // theta    (in DEG bounded within [0, 360])
    map_priv_angle(
        &current_rotangle);  // rotangle (in DEG bounded within [0, 360])
    ...
    printf("%-30s %-10.6f %-10.6f %-10.6f\n",
           "current_axisangle (1,2,3): ", current_phi, current_theta,
           current_rotangle);
#endif
    ...
    printf("%-30s %-10.6f %-10.6f %-10.6f %-10.6f\n",
           "current_q (w,x,y,z): ", current_q.w, current_q.x, current_q.y,
           current_q.z);
#endif
    ...
    target_q.w = quat_torque.w * current_q.w - quat_torque.x * current_q.x -
                 quat_torque.y * current_q.y -
                 quat_torque.z * current_q.z;  // w
    target_q.x = quat_torque.w * current_q.x + quat_torque.x * current_q.w +
                 quat_torque.y * current_q.z -
                 quat_torque.z * current_q.y;  // x
    target_q.y = quat_torque.w * current_q.y + quat_torque.y * current_q.w +
                 quat_torque.z * current_q.x -
                 quat_torque.x * current_q.z;  // y
    target_q.z = quat_torque.w * current_q.z + quat_torque.z * current_q.w +
                 quat_torque.x * current_q.y -
                 quat_torque.y * current_q.x;  // z
#if defined(PRINT_GRAD_ROTATION_GENES)
    printf("\n%s\n",
           "----------------------------------------------------------");
    printf("%-30s %-10.6f %-10.6f %-10.6f %-10.6f\n",
    ...
    if (is_theta_gt_pi == false) {
      ...
      target_phi =
          fmod((atan2(-rotaxis_y, -rotaxis_x) + PI_TIMES_2), PI_TIMES_2);
    ...
    */
    grad_phi =
        orientation_scaling *
        (fmod(target_phi - current_phi + PI_FLOAT, PI_TIMES_2) - PI_FLOAT);
    grad_theta =
        orientation_scaling *
        (fmod(target_theta - current_theta + PI_FLOAT, PI_TIMES_2) - PI_FLOAT);
    grad_rotangle =
        orientation_scaling *
        (fmod(target_rotangle - current_rotangle + PI_FLOAT, PI_TIMES_2) -
         PI_FLOAT);
    ...
    */
    ...
    if (index_theta <= 0) {
      ...
      dependence_on_theta = dependence_on_theta_const
    ...
    dependence_on_theta =
        (Y0_theta * (X1_theta - X_theta) + Y1_theta * (X_theta - X0_theta)) *
        inv_angle_delta;
    ...
    if (index_rotangle <= 0) {
      ...
      dependence_on_rotangle = dependence_on_rotangle_const
    ...
    dependence_on_rotangle = (Y0_rotangle * (X1_rotangle - X_rotangle) +
                              Y1_rotangle * (X_rotangle - X0_rotangle)) *
                             inv_angle_delta;
    ...
    // gradients)
    gradient_genotype[3] = native_divide(grad_phi, (dependence_on_theta *
                                                    dependence_on_rotangle)) *
                           DEG_TO_RAD;
    gradient_genotype[4] =
        native_divide(grad_theta, dependence_on_rotangle) * DEG_TO_RAD;
  ...
  for (uint rotbond_id = tidx; rotbond_id < dockpars_num_of_genes - 6;
    ...
    */
    ...
    printf("\n");
    printf("%-15s \n\t %-10.6f %-10.6f %-10.6f\n",
           "unitvec: ", rotation_unitvec.x, rotation_unitvec.y,
           rotation_unitvec.z);
#endif
    ...
    for (uint rotable_atom_cnt = 0;
         ...
         rotable_atom_cnt++) {
      uint lig_atom_id = rotbonds_atoms_const[MAX_NUM_OF_ATOMS * rotbond_id +
                                              rotable_atom_cnt];
      ...
      // printf("%-15s %-10u\n", "atom_id: ", lig_atom_id);
      printf(
          "\t %-15s %-10.6f %-10.6f %-10.6f \t %-15s %-10.6f %-10.6f %-10.6f\n",
...
// printf("%-15s %-10.6f %-10.6f %-10.6f\n", "atom_force: ", atom_force.x,
// atom_force.y, atom_force.z); printf("%-15s %-10.6f %-10.6f %-10.6f\n",
// "torque_tor: ", torque_tor.x, torque_tor.y, torque_tor.z);
    ...
    gradient_genotype[rotbond_id + 6] =
        torque_on_axis * DEG_TO_RAD /*(M_PI / 180.0f)*/;
