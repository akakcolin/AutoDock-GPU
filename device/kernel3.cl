...
__kernel void
    ...
    perform_LS(
        ...
        uint dockpars_num_of_lsentities, float dockpars_rho_lower_bound,
        float dockpars_base_dmov_mul_sqrt3, float dockpars_base_dang_mul_sqrt3,
        uint dockpars_cons_limit, uint dockpars_max_num_of_iters,
...
// num_of_lsentities is selected instead of the first one (with ID 0).
{
  ...
  while ((iteration_cnt < dockpars_max_num_of_iters) &&
         (rho > dockpars_rho_lower_bound)) {
    // New random deviate
    for (gene_counter = tidx; gene_counter < dockpars_num_of_genes;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
#ifdef SWAT3
      genotype_deviate[gene_counter] =
          rho * (2 * gpu_randf(dockpars_prng_states) - 1) *
          (gpu_randf(dockpars_prng_states) < gene_scale);
      ...
      // Orientation and torsion genes
        ...
        if (gene_counter < 6) {
          genotype_deviate[gene_counter] *=
              dockpars_base_dang_mul_sqrt3 * lig_scale;
        } else {
          genotype_deviate[gene_counter] *=
              dockpars_base_dang_mul_sqrt3 * gene_scale;
        }
      }
#else
      genotype_deviate[gene_counter] =
          rho * (2 * gpu_randf(dockpars_prng_states) - 1) *
          (gpu_randf(dockpars_prng_states) < 0.3);
    ...
    gpu_calc_energy(
        ...
        genotype_candidate, &candidate_energy, &run_id,
        // Some OpenCL compilers don't allow declaring
        // local variables within non-kernel functions.
        // These local variables must be declared in a kernel,
        // and then passed to non-kernel functions.
    ...
    } else  // If candidate is worser, check the opposite direction
    {
      ...
      // =================================================================
      gpu_calc_energy(
          dockpars_rotbondlist_length, dockpars_num_of_atoms,
    ...
    if (tidx == 0) {
      ...
      if (cons_succ >= dockpars_cons_limit) {
        rho *= LS_EXP_FACTOR;
        cons_succ = 0;
      } else if (cons_fail >= dockpars_cons_limit) {
        rho *= LS_CONT_FACTOR;
        cons_fail = 0;
      }
    }
