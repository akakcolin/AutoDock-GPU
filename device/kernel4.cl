...
// if defined, new (experimental) GA genotype mutation, similar to experimental
// SW move, dependent on nr of atoms on torsions of ligand is used, not ready
// yet ... #define GA_MUTATION_TEST

//#define DEBUG_ENERGY_KERNEL4

__kernel void
    __attribute__((reqd_work_group_size(NUM_OF_THREADS_PER_BLOCK, 1, 1)))
    gpu_gen_and_eval_newpops(
        ...
        int dockpars_gridsize_x, int dockpars_gridsize_y,
        int dockpars_gridsize_z,
        // g1 = gridsize_x
        uint dockpars_gridsize_x_times_y,  // g2 = gridsize_x * gridsize_y
        uint dockpars_gridsize_x_times_y_times_z,  // g3 = gridsize_x *
                                                   // gridsize_y * gridsize_z
        ...
        int dockpars_num_of_genes, float dockpars_tournament_rate,
        float dockpars_crossover_rate, float dockpars_mutation_rate,
        float dockpars_abs_max_dmov, float dockpars_abs_max_dang,
...
// The GPU global function
{
  ...
  if ((get_group_id(0) % dockpars_pop_size) == 0) {
    gpu_perform_elitist_selection(
        dockpars_pop_size, dockpars_energies_current, dockpars_energies_next,
        dockpars_evals_of_new_entities, dockpars_num_of_genes,
        dockpars_conformations_next, dockpars_conformations_current,
        best_energies, best_IDs, best_ID);
  } else {
    ...
    for (gene_counter = tidx; gene_counter < 10;
    ...
    for (gene_counter = tidx; gene_counter < 4;
         gene_counter +=
         NUM_OF_THREADS_PER_BLOCK) {  // it is not ensured that the four
                                      // candidates will be different...

      parent_candidates[gene_counter] =
          (int)(dockpars_pop_size *
                randnums[gene_counter]);  // using randnums[0..3]
      candidate_energies[gene_counter] =
          dockpars_energies_current[run_id * dockpars_pop_size +
                                    parent_candidates[gene_counter]];
    }

    barrier(CLK_LOCAL_MEM_FENCE);

    for (gene_counter = tidx; gene_counter < 2;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
      // Notice: dockpars_tournament_rate was scaled down to [0,1] in host
      // to reduce number of operations in device
      if (candidate_energies[2 * gene_counter] <
          candidate_energies[2 * gene_counter + 1])
        if (/*100.0f**/ randnums[4 + gene_counter] <
            dockpars_tournament_rate) {  // using randnum[4..5]
          parents[gene_counter] = parent_candidates[2 * gene_counter];
        } else {
          parents[gene_counter] = parent_candidates[2 * gene_counter + 1];
        }
      else if (/*100.0f**/ randnums[4 + gene_counter] <
               dockpars_tournament_rate) {
        parents[gene_counter] = parent_candidates[2 * gene_counter + 1];
      } else {
        parents[gene_counter] = parent_candidates[2 * gene_counter];
      }
    ...
    if (/*100.0f**/ randnums[6] < dockpars_crossover_rate)  // Using randnums[6]
    {
      for (gene_counter = tidx; gene_counter < 2;
           gene_counter += NUM_OF_THREADS_PER_BLOCK) {
        // Using randnum[7..8]
        covr_point[gene_counter] =
            (int)((dockpars_num_of_genes - 1) * randnums[7 + gene_counter]);
      }
      ...
      event_t ev =
          async_work_group_copy(offspring_genotype,
                                dockpars_conformations_current +
                                    (run_id * dockpars_pop_size + parents[0]) *
                                        GENOTYPE_LENGTH_IN_GLOBMEM,
    ...
    float rot_scale = native_sqrt(native_divide((float)dockpars_num_of_genes,
                                                (float)dockpars_num_of_atoms));
#endif
    for (gene_counter = tidx; gene_counter < dockpars_num_of_genes;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
      // Notice: dockpars_mutation_rate was scaled down to [0,1] in host
      // to reduce number of operations in device
      if (/*100.0f**/ gpu_randf(dockpars_prng_states) <
        ...
        // Orientation and torsion genes
          ...
          if (gene_counter < 6) {
            offspring_genotype[gene_counter] +=
                pmone * dockpars_abs_max_dang * rot_scale;
          } else {
        ...
        if (gene_counter < 3) {
          offspring_genotype[gene_counter] +=
              dockpars_abs_max_dmov * (2 * gpu_randf(dockpars_prng_states) - 1);
        }
        // Orientation and torsion genes
        else {
          offspring_genotype[gene_counter] +=
              dockpars_abs_max_dang * (2 * gpu_randf(dockpars_prng_states) - 1);
          map_angle(&(offspring_genotype[gene_counter]));
        }
#endif
      }
    ...
    gpu_calc_energy(
        ...
        offspring_genotype, &energy, &run_id,
        // Some OpenCL compilers don't allow declaring
        // local variables within non-kernel functions.
        // These local variables must be declared in a kernel,
        // and then passed to non-kernel functions.
    ...
    if (tidx == 0) {
      ...
      printf("%-18s [%-5s]---{%-5s}   [%-10.8f]---{%-10.8f}\n",
             "-ENERGY-KERNEL4-", "GRIDS", "INTRA", partial_interE[0],
             partial_intraE[0]);
#endif
    }
    ...
    event_t ev2 =
        async_work_group_copy(dockpars_conformations_next +
                                  GENOTYPE_LENGTH_IN_GLOBMEM * get_group_id(0),
                              offspring_genotype, dockpars_num_of_genes, 0);
