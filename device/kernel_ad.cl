...
// instead of the first one (with ID 0).
{
  ...
  if (tidx == 0) {
    ...
    entity_id =
        get_group_id(0) -
        run_id * dockpars_num_of_lsentities;  // modulus in different form
    ...
    if (entity_id == 0) {
      ...
      if (100.0f * gpu_randf(dockpars_prng_states) > dockpars_lsearch_rate) {
        entity_id =
            dockpars_num_of_lsentities;  // AT - Should this be
  ...
  for (uint i = tidx; i < dockpars_num_of_genes;
...
// Printing number of ADADELTA iterations
    ...
    gpu_calc_energrad(
        ...
        // Derived from autodockdev/maps.py
        ,
        dockpars_num_of_genes, gradient_x, gradient_y, gradient_z,
        accumulator_x, accumulator_y, accumulator_z, gradient);
// =============================================================
// =============================================================
// =============================================================
    ...
    for (uint i = tidx; i < dockpars_num_of_genes;
         i += NUM_OF_THREADS_PER_BLOCK) {
      if (energy <
          best_energy)  // we need to be careful not to change best_energy until
                        // we had a chance to update the whole array
        best_genotype[i] = genotype[i];

      // Accummulating gradient^2 (eq.8 in the paper)
      // square_gradient corresponds to E[g^2]
      square_gradient[i] =
          RHO * square_gradient[i] + (1.0f - RHO) * gradient[i] * gradient[i];

      // Computing update (eq.9 in the paper)
      delta[i] =
          -1.0f * gradient[i] *
          native_sqrt(native_divide((float)(square_delta[i] + EPSILON),
                                    (float)(square_gradient[i] + EPSILON)));

      // Accummulating update^2
      // square_delta corresponds to E[dx^2]
      square_delta[i] =
          RHO * square_delta[i] + (1.0f - RHO) * delta[i] * delta[i];
      ...
      for (uint i = 0; i < dockpars_num_of_genes; i++) {
        if (i == 0) {
          ...
          printf("%13s %20s %15s %15s %15s\n", "gene", "sq_grad", "delta",
                 "sq_delta", "new.genotype");
        }
        printf("%13u %20.6f %15.6f %15.6f %15.6f\n", i, square_gradient[i],
               delta[i], square_delta[i], genotype[i]);
      }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
#endif
    ...
    defined(PRINT_ADADELTA_MINIMIZER_ENERGY_EVOLUTION)
      ...
      printf("%-18s [%-5s]---{%-5s}   [%-10.7f]---{%-10.7f}\n",
             "-ENERGY-KERNEL7-", "GRIDS", "INTRA", partial_interE[0],
             partial_intraE[0]);
#endif
#ifdef ADADELTA_AUTOSTOP
      if (cons_succ >= 4) {
    ...
    barrier(
        CLK_LOCAL_MEM_FENCE);  // making sure that iteration_cnt is up-to-date
#ifdef ADADELTA_AUTOSTOP
  ...
  for (uint gene_counter = tidx + 3; gene_counter < dockpars_num_of_genes;
  ...
  if (tidx == 0) {
    ...
    printf(
        "-------> End of ADADELTA minimization cycle, num of energy evals: %u, "
        "final energy: %.6f\n",
