...
__kernel void __attribute__((reqd_work_group_size(NUM_OF_THREADS_PER_BLOCK, 1,
                                                  1)))
gradient_minFire(
    int dockpars_num_of_atoms, int dockpars_num_of_atypes,
    int dockpars_num_of_map_atypes, int dockpars_num_of_intraE_contributors,
    ...
    int dockpars_rotbondlist_length, float dockpars_coeff_elec,
    float dockpars_coeff_desolv,
    __global float* restrict dockpars_conformations_next,
    __global float* restrict dockpars_energies_next,
    __global int* restrict dockpars_evals_of_new_entities,
    __global uint* restrict dockpars_prng_states, int dockpars_pop_size,
    int dockpars_num_of_genes, float dockpars_lsearch_rate,
    uint dockpars_num_of_lsentities, uint dockpars_max_num_of_iters,
    float dockpars_qasp, float dockpars_smooth,
...
// The GPU global function performs gradient-based minimization on (some)
// entities of conformations_next. The number of OpenCL compute units (CU) which
// should be started equals to num_of_minEntities*num_of_runs. This way the first
// num_of_lsentities entity of each population will be subjected to local search
...
// subjected to local search, the entity with ID num_of_lsentities is selected
// instead of the first one (with ID 0).
{
  // -----------------------------------------------------------------------------
  // -----------------------------------------------------------------------------
  // -----------------------------------------------------------------------------
  ...
  if (tidx == 0) {
    ...
    */
    ...
    defined(PRINT_FIRE_MINIMIZER_ENERGY_EVOLUTION)
    printf("\n");
  ...
  event_t ev = async_work_group_copy(
      ...
      dockpars_conformations_next +
          (run_id * dockpars_pop_size + entity_id) * GENOTYPE_LENGTH_IN_GLOBMEM,
...
// Enable this for debugging FIRE from a defined initial genotype
  ...
  // =============================================================
  gpu_calc_energy(
      dockpars_rotbondlist_length, dockpars_num_of_atoms, dockpars_gridsize_x,
      dockpars_gridsize_y, dockpars_gridsize_z,
      // g1 = gridsize_x
      dockpars_gridsize_x_times_y,          // g2 = gridsize_x * gridsize_y
      dockpars_gridsize_x_times_y_times_z,  // g3 = gridsize_x * gridsize_y *
                                            // gridsize_z
      dockpars_fgrids, dockpars_num_of_atypes, dockpars_num_of_map_atypes,
      dockpars_num_of_intraE_contributors, dockpars_grid_spacing,
      dockpars_coeff_elec, dockpars_qasp, dockpars_coeff_desolv,
      dockpars_smooth,
      ...
      calc_coords, partial_energies,
#if defined(DEBUG_ENERGY_KERNEL)
      partial_interE, partial_intraE,
#endif
#if 0
			true,
#endif
      kerconst_interintra, kerconst_intracontrib, kerconst_intra,
      kerconst_rotlist, kerconst_conform);
  // =============================================================
  ...
  gpu_calc_gradient(
      ...
      kerconst_interintra, kerconst_intracontrib, kerconst_intra,
      kerconst_rotlist, kerconst_conform, rotbonds_const, rotbonds_atoms_const,
      num_rotating_atoms_per_rotbond_const, angle_const,
      ...
      // Derived from autodockdev/maps.py
      ,
      dockpars_num_of_genes, i_gradient_x, i_gradient_y, i_gradient_z,
      f_gradient_x, f_gradient_y, f_gradient_z, gradient);
  // =============================================================
  ...
  for (uint gene_counter = tidx; gene_counter < dockpars_num_of_genes;
       gene_counter += NUM_OF_THREADS_PER_BLOCK) {
    gradient_tmp[gene_counter] =
        gradient[gene_counter] * gradient[gene_counter];
  }
  barrier(CLK_LOCAL_MEM_FENCE);
...
// Printing number of FIRE iterations
    ...
    for (uint gene_counter = tidx; gene_counter < dockpars_num_of_genes;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
      candidate_genotype[gene_counter] =
          genotype[gene_counter] + dt * velocity[gene_counter];
    }
...
// of genotypes, energy, and gradients
    ...
    gpu_calc_energrad(
        ...
        // These local variables must be declared in a kernel,
        // and then passed to non-kernel functions.
        candidate_genotype, &candidate_energy, &run_id,
        ...
        kerconst_interintra, kerconst_intracontrib, kerconst_intra,
        kerconst_rotlist, kerconst_conform, rotbonds_const,
        rotbonds_atoms_const, num_rotating_atoms_per_rotbond_const, angle_const,
        dependence_on_theta_const,
        dependence_on_rotangle_const
        // Gradient-related arguments
        // Calculate gradients (forces) for intermolecular energy
        ...
        ,
        dockpars_num_of_genes, i_gradient_x, i_gradient_y, i_gradient_z,
        f_gradient_x, f_gradient_y, f_gradient_z, candidate_gradient);
    // =============================================================
    // =============================================================
    // =============================================================
    ...
    for (uint gene_counter = tidx; gene_counter < dockpars_num_of_genes;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
      // Calculating power
      power_tmp[gene_counter] =
          -candidate_gradient[gene_counter] * velocity[gene_counter];
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (/*(get_group_id(0) == 0) &&*/ (tidx == 0)) {
      ...
      for (uint i = 0; i < dockpars_num_of_genes; i++) {
        if (i == 0) {
          ...
          printf("%13s %13s %5s %15s %21s %15s\n", "gene_id", "genotype", "|",
                 "gradient", "(autodockdevpy units)", "velocity");
        }
        printf("%13u %13.6f %5s %15.6f %21.6f %15.6f\n", i, genotype[i], "|",
               gradient[i],
               (i < 3) ? (gradient[i] / dockpars_grid_spacing)
      ...
      for (uint i = 0; i < dockpars_num_of_atoms; i++) {
        if (i == 0) {
          ...
          printf("%12s %12s %12s %12s\n", "atom_id", "coords.x", "coords.y",
                 "coords.z");
        }
        printf("%12u %12.6f %12.6f %12.6f\n", i, calc_coords_x[i],
               calc_coords_y[i], calc_coords_z[i]);
      }
      printf("\n");
#endif
    }
    ...
    for (uint gene_counter = tidx; gene_counter < dockpars_num_of_genes;
         gene_counter += NUM_OF_THREADS_PER_BLOCK) {
      velocity_tmp[gene_counter] =
          velocity[gene_counter] * velocity[gene_counter];
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (uint gene_counter = tidx; gene_counter < dockpars_num_of_genes;
      ...
      velocity[gene_counter] = (1 - alpha) * velocity[gene_counter] -
                               gradient[gene_counter] * velnorm_div_gradnorm;
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    defined(PRINT_FIRE_MINIMIZER_ENERGY_EVOLUTION)
      ...
      printf("%-18s [%-5s]---{%-5s}   [%-10.7f]---{%-10.7f}\n",
             "-ENERGY-KERNEL6-", "GRIDS", "INTRA", partial_interE[0],
             partial_intraE[0]);
#endif
  ...
  if (tidx == 0) {
    dockpars_evals_of_new_entities[run_id * dockpars_pop_size + entity_id] +=
        iteration_cnt;
    ...
    printf("\n");
    printf(
        "Termination criteria: ( dt >= %10.10f ) OR ( #fire-iters >= %-3u )\n",
        DT_MIN, dockpars_max_num_of_iters);
    printf(
        "-------> End of FIRE minimization cycle, num of energy evals: %u, "
        "final energy: %.6f\n",
  ...
  event_t ev2 = async_work_group_copy(
      dockpars_conformations_next +
          (run_id * dockpars_pop_size + entity_id) * GENOTYPE_LENGTH_IN_GLOBMEM,
      best_genotype, dockpars_num_of_genes, 0);
