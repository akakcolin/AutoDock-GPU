...
// This could be enabled back for double checking
    ...
    if (tidx == 0) {
      // Finding maximum of the absolute value for the three translation
      // gradients
      ...
      max_trans_stepsize = native_divide(
          native_divide(MAX_DEV_TRANSLATION, dockpars_grid_spacing),
          max_trans_grad);
    ...
    for (uint i = tidx; i < dockpars_num_of_genes - 6;
...
// This could be enabled back for details
    ...
    for (uint i = tidx; i < dockpars_num_of_genes;
      ...
      printf("%13u %13.6f %5s %15.6f %5s %18.6f\n", i, genotype[i], "|",
             gradient[i], "|", candidate_genotype[i]);
...
// This could be enabled back for double checking
    ...
    gpu_calc_energy(
        ...
        candidate_genotype,
        /*genotype,*/ /*WARNING: use "genotype" ONLY to reproduce results*/
        &candidate_energy, &run_id,
        // Some OpenCL compilers don't allow declaring
        // local variables within non-kernel functions.
        // These local variables must be declared in a kernel,
        // and then passed to non-kernel functions.
      ...
      for (uint i = 0; i < dockpars_num_of_genes; i++) {
        if (i == 0) {
          printf("\n%s\n",
                 "----------------------------------------------------------");
          printf("%13s %13s %5s %15s %15s\n", "gene_id",
                 "cand-gene.value" /* "gene.value"*/, "|", "gene.grad",
                 "(autodockdevpy units)");
        }
        printf("%13u %13.6f %5s %15.6f %15.6f\n", i,
               candidate_genotype[i] /*genotype[i]*/, "|", gradient[i],
    ...
    if (candidate_energy < energy) {
      for (uint i = tidx; i < dockpars_num_of_genes;
        ...
        // genotype[i], gradient[i], candidate_genotype[i]);
    ...
    if (tidx == 0) {
      ...
      printf("%-18s [%-5s]---{%-5s}   [%-10.7f]---{%-10.7f}\n",
             "-ENERGY-KERNEL5-", "GRIDS", "INTRA", partial_interE[0],
             partial_intraE[0]);
#endif
    }
    barrier(CLK_LOCAL_MEM_FENCE);
  ...
  if (tidx == 0) {
    ...
    printf(
        "-------> End of SD minimization cycle, num of energy evals: %u, final "
        "energy: %.6f\n",
  ...
  event_t ev2 = async_work_group_copy(
      dockpars_conformations_next +
          (run_id * dockpars_pop_size + entity_id) * GENOTYPE_LENGTH_IN_GLOBMEM,
      genotype, dockpars_num_of_genes, 0);
