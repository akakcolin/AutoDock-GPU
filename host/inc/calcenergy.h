...
// and the genetic algorithm, or they are pointers of GPU (ADM FPGA) memory
// areas used for storing different data such as the current and the next
// population genotypes and energies, the grids, the evaluation counters and the
...
processligand.h/Liganddata->atom_rotbonds
        // "atom_rotbonds": array that contains the rotatable bonds - atoms
assignment.
        // If the element atom_rotbonds[atom index][rotatable bond index] is
equal to 1,
        // it means,that the atom must be rotated if the bond rotates. A 0 means
the opposite. int  rotbonds_atoms [MAX_NUM_OF_ATOMS * MAX_NUM_OF_ROTBONDS];
