...
#define getvalue_4Darr(mempoi, grinf, t, z, y, x)                        \
  *(mempoi +                                                             \
    4 * ((grinf).size_xyz[0] *                                           \
             (y + (grinf).size_xyz[1] * (z + (grinf).size_xyz[2] * t)) + \
         x))
#define getvalue_4Darr_withsize(mempoi, gridsize_xyz, t, z, y, x)              \
  *(mempoi +                                                                   \
    4 * (gridsize_xyz[0] * (y + gridsize_xyz[1] * (z + gridsize_xyz[2] * t)) + \
         x))
// The macro helps to access the grid point values
// which were read from external files with get_gridvalues function.
// The first parameter is a pointer which points to the memory area storing the
// data. The second one is the corresponding grid info (parameter of get_gridinfo
// function). The other parameters are the type index, z, y and x coordinates of
// the grid point.

typedef struct
// Struct which can contain all the important informations which derives from
// .gpf and .xyz files.
{
#ifndef _WIN32
  char* grid_file_path;  // Added to store the full path of the grid file
#else
  char grid_file_path[2 * _MAX_DIR];  // Added to store the full path of the
                                      // grid file
#endif
  ...
  char grid_types[MAX_NUM_OF_ATYPES + 2]
                 [4];  // The additional two are the electrostatic and the
                       // desolvation types
  int num_of_atypes;
