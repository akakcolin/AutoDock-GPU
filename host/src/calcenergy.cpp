...
int prepare_const_fields_for_gpu(
    Liganddata* myligand_reference, Dockpars* mypars, float* cpu_ref_ori_angles,
...
//		      Element i corresponds to atom with atom ID i in
//myligand_reference. atom_types_const: stores the ligand atom type IDs
// according to myligand_reference. 		    Element i corresponds to atom with ID i in
//myligand_reference. intraE_contributors_const: each three contiguous items
// describe an intramolecular contributor. 		         The first two elements store the atom
//ID of the contributors according to myligand_reference. 		         The third element is
//0, if no H-bond can occur between the two atoms, and 1, if it can.
// VWpars_AC_const: stores the A or C van der Waals parameters.
//                 The element i*MAX_NUM_OF_ATYPES+j and j*MAX_NUM_OF_ATYPES+i
//                 corresponds to A or C in case of
//		   H-bond for atoms with type ID i and j (according to
//myligand_reference). VWpars_BD_const: stores the B or D van der Waals
// parameters similar to VWpars_AC_const. dspars_S_const: stores the S
// desolvation parameters. 		  The element i corresponds to the S parameter of atom
//with type ID i 		  according to myligand_reference. rotlist_const: stores the data
// describing the rotations for conformation calculation. 		 Each element describes
//one rotation, and the elements are in a proper order,
//               considering that NUM_OF_THREADS_PER_BLOCK rotations will be
//               performed in
//		 parallel (that is, each block of contiguous
//NUM_OF_THREADS_PER_BLOCK pieces of elements describe rotations that can 		 be
//performed simultaneously). 		 One element is a 32 bit integer, with bit 0 in the
//LSB position. 		 Bit 7-0 describe the atom ID of the atom to be rotated
//(according to myligand_reference). 		 Bit 15-7 describe the rotatable bond ID of
//the bond around which the atom is to be rotated (if this is not a general
//rotation) 				 (bond ID is according to myligand_reference). 		 If bit 16 is 1, this
//is the first rotation of the atom. 		 If bit 17 is 1, this is a general rotation
//(so rotbond ID has to be ignored). 		 If bit 18 is 1, this is a "dummy" rotation,
//that is, no rotation can be performed in this cycle 	         (considering the other
//rotations which are being carried out in this period). ref_coords_x_const:
// stores the x coordinates of the reference ligand atoms. 		      Element i corresponds
//to the x coordinate of 					  atom with atom ID i (according to myligand_reference).
// ref_coords_y_const: stores the y coordinates of the reference ligand atoms
// similarly to ref_coords_x_const. ref_coords_z_const: stores the z coordinates
// of the reference ligand atoms similarly to ref_coords_x_const.
// rotbonds_moving_vectors_const: stores the coordinates of rotatable bond
// moving vectors. Element i, i+1 and i+2 (where i%3=0) 								 correspond to the moving
//vector coordinates x, y and z of rotbond ID i, respectively 								 (according to
//myligand_reference). rotbonds_unit_vectors_const: stores the coordinates of
// rotatable bond unit vectors similarly to rotbonds_moving_vectors_const.
// ref_orientation_quats_const: stores the quaternions describing the reference
// orientations for each run. Element i, i+1, i+2 							   and i+3 (where i%4=0)
//correspond to the quaternion coordinates q, x, y and z of reference
//							   orientation for run i,
//respectively.
{
  int i, j;
  ...
  // "atom_rotbonds": array that contains the rotatable bonds - atoms
  // assignment. If the element atom_rotbonds[atom index][rotatable bond index]
  // is equal to 1, it means,that the atom must be rotated if the bond rotates.
  ...
  if (myligand_reference->num_of_intraE_contributors >
    ...
    for (j = i + 1; j < myligand_reference->num_of_atoms; j++) {
      if (myligand_reference->intraE_contributors[i][j] == 1) {
        ...
        *intpoi = (int)j;
        ...
        if (is_H_bond(myligand_reference->atom_types[type_id1],
                      myligand_reference->atom_types[type_id2]) != 0)
          *intpoi = (int)1;
        else
          *intpoi = (int)0;
        intpoi++;
  ...
  for (i = 0; i < myligand_reference->num_of_atypes; i++)
    for (j = 0; j < myligand_reference->num_of_atypes; j++) {
      if (is_H_bond(myligand_reference->atom_types[i],
                    myligand_reference->atom_types[j]) != 0) {
  ...
  for (i = 0; i < myligand_reference->num_of_rotbonds; i++)
    for (j = 0; j < 3; j++) {
      rotbonds_moving_vectors[3 * i + j] =
          myligand_reference->rotbonds_moving_vectors[i][j];
      rotbonds_unit_vectors[3 * i + j] =
          myligand_reference->rotbonds_unit_vectors[i][j];
    }

  // reference orientation quaternions
  for (i = 0; i < mypars->num_of_runs; i++) {
    // printf("Pregenerated angles for run %d: %f %f %f\n", i,
    // cpu_ref_ori_angles[3*i], cpu_ref_ori_angles[3*i+1],
    // cpu_ref_ori_angles[3*i+2]);
    ...
    ref_orientation_quats[4 * i + 0] =
        sinf(genrotangle / 2.0f) * sinf(theta) * cosf(phi);  // x
    ref_orientation_quats[4 * i + 1] =
        sinf(genrotangle / 2.0f) * sinf(theta) * sinf(phi);  // y
    ref_orientation_quats[4 * i + 2] =
        sinf(genrotangle / 2.0f) * cosf(theta);                   // z
    ...
    /*
                    ...
                    ref_orientation_quats[4*i]   = sqrt(1-u1) * sinf(2*PI*u2);
       //q ref_orientation_quats[4*i+1] = sqrt(1-u1) * cosf(2*PI*u2);	//x
                    ref_orientation_quats[4*i+2] = sqrt(u1)   * sinf(2*PI*u3);
       //y ref_orientation_quats[4*i+3] = sqrt(u1)   * cosf(2*PI*u3);	//z
    */
    // printf("Precalculated quaternion for run %d: %f %f %f %f\n", i,
    // ref_orientation_quats[4*i], ref_orientation_quats[4*i+1],
    // ref_orientation_quats[4*i+2], ref_orientation_quats[4*i+3]);
  }
  ...
  for (i = 0; i < myligand_reference->num_of_rotbonds; i++) {
    ...
    rotbonds[2 * i + 1] =
        myligand_reference->rotbonds[i][1];  // id of second atom
  }
  ...
  for (i = 0; i < myligand_reference->num_of_rotbonds; i++) {
    ...
    for (j = 0; j < myligand_reference->num_of_atoms; j++) {
      /*
      rotbonds_atoms [MAX_NUM_OF_ATOMS*i+j] = myligand_reference->atom_rotbonds
      [j][i]; //
      */
  ...
  /*
  for (m=0;m<MAX_NUM_OF_ATOMS;m++){ KerConst->atom_charges_const[m] =
  atom_charges[m]; } for (m=0;m<MAX_NUM_OF_ATOMS;m++){
  KerConst->atom_types_const[m]   = atom_types[m]; } for
  (m=0;m<3*MAX_INTRAE_CONTRIBUTORS;m++){ KerConst->intraE_contributors_const[m]
  = intraE_contributors[m]; } for (m=0;m<ATYPE_NUM;m++){ KerConst->reqm_const[m]
  = reqm[m]; } for (m=0;m<ATYPE_NUM;m++){ KerConst->reqm_hbond_const[m] =
  reqm_hbond[m]; } for (m=0;m<ATYPE_NUM;m++){
  KerConst->atom1_types_reqm_const[m] = atom1_types_reqm[m]; } for
  (m=0;m<ATYPE_NUM;m++){ KerConst->atom2_types_reqm_const[m] =
  atom2_types_reqm[m]; } for (m=0;m<MAX_NUM_OF_ATYPES*MAX_NUM_OF_ATYPES;m++){
  KerConst->VWpars_AC_const[m]   = VWpars_AC[m]; } for
  (m=0;m<MAX_NUM_OF_ATYPES*MAX_NUM_OF_ATYPES;m++){ KerConst->VWpars_BD_const[m]
  = VWpars_BD[m]; } for (m=0;m<MAX_NUM_OF_ATYPES;m++)		   {
  KerConst->dspars_S_const[m]    = dspars_S[m]; } for
  (m=0;m<MAX_NUM_OF_ATYPES;m++)		   { KerConst->dspars_V_const[m]    =
  dspars_V[m]; } for (m=0;m<MAX_NUM_OF_ROTATIONS;m++)		   {
  KerConst->rotlist_const[m]     = rotlist[m]; } for
  (m=0;m<MAX_NUM_OF_ATOMS;m++)		   { KerConst->ref_coords_x_const[m]=
  ref_coords_x[m]; } for (m=0;m<MAX_NUM_OF_ATOMS;m++)		   {
  KerConst->ref_coords_y_const[m]= ref_coords_y[m]; } for
  (m=0;m<MAX_NUM_OF_ATOMS;m++)		   { KerConst->ref_coords_z_const[m]=
  ref_coords_z[m]; } for (m=0;m<3*MAX_NUM_OF_ROTBONDS;m++){
  KerConst->rotbonds_moving_vectors_const[m]= rotbonds_moving_vectors[m]; } for
  (m=0;m<3*MAX_NUM_OF_ROTBONDS;m++){ KerConst->rotbonds_unit_vectors_const[m]  =
  rotbonds_unit_vectors[m]; } for (m=0;m<4*MAX_NUM_OF_RUNS;m++)    {
  KerConst->ref_orientation_quats_const[m]  = ref_orientation_quats[m]; }
  */

  for (m = 0; m < MAX_NUM_OF_ATOMS; m++) {
    KerConst_interintra->atom_charges_const[m] = atom_charges[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATOMS; m++) {
    KerConst_interintra->atom_types_const[m] = atom_types[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATOMS; m++) {
    KerConst_interintra->atom_types_map_const[m] =
        myligand_reference->atom_map_to_fgrids[m];
  }

  for (m = 0; m < 3 * MAX_INTRAE_CONTRIBUTORS; m++) {
    KerConst_intracontrib->intraE_contributors_const[m] =
        intraE_contributors[m];
  }
  ...
  for (m = 0; m < ATYPE_NUM; m++) {
    KerConst_intra->atom1_types_reqm_const[m] = atom1_types_reqm[m];
  }
  for (m = 0; m < ATYPE_NUM; m++) {
    KerConst_intra->atom2_types_reqm_const[m] = atom2_types_reqm[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATYPES * MAX_NUM_OF_ATYPES; m++) {
    KerConst_intra->VWpars_AC_const[m] = VWpars_AC[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATYPES * MAX_NUM_OF_ATYPES; m++) {
    KerConst_intra->VWpars_BD_const[m] = VWpars_BD[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATYPES; m++) {
    KerConst_intra->dspars_S_const[m] = dspars_S[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATYPES; m++) {
    KerConst_intra->dspars_V_const[m] = dspars_V[m];
  }
  ...
  for (m = 0; m < MAX_NUM_OF_ROTATIONS; m++) {
    ...
    /*		if(m!=0 && m%myligand_reference->num_of_atoms==0)
                    ...
                    if(m!=0 && m%NUM_OF_THREADS_PER_BLOCK==0)
                            printf("===\n");
                    printf("%i (%i): %i -> atom_id: %i, dummy: %i, first: %i,
       genrot: %i, rotbond_id:
       %i\n",m,m%NUM_OF_THREADS_PER_BLOCK,rotlist[m],rotlist[m] &
       RLIST_ATOMID_MASK, rotlist[m] & RLIST_DUMMY_MASK,rotlist[m] &
       RLIST_FIRSTROT_MASK,rotlist[m] & RLIST_GENROT_MASK,(rotlist[m] &
       RLIST_RBONDID_MASK) >> RLIST_RBONDID_SHIFT);*/
  }
  ...
  for (m = 0; m < 3 * MAX_NUM_OF_ROTBONDS; m++) {
    KerConst_conform->rotbonds_moving_vectors_const[m] =
        rotbonds_moving_vectors[m];
  }
  for (m = 0; m < 3 * MAX_NUM_OF_ROTBONDS; m++) {
    KerConst_conform->rotbonds_unit_vectors_const[m] = rotbonds_unit_vectors[m];
  }
  for (m = 0; m < 4 * MAX_NUM_OF_RUNS; m++) {
    KerConst_conform->ref_orientation_quats_const[m] = ref_orientation_quats[m];
  }
  ...
  for (m = 0; m < 2 * MAX_NUM_OF_ROTBONDS; m++) {
    KerConst_grads->rotbonds[m] = rotbonds[m];
  }
  for (m = 0; m < MAX_NUM_OF_ATOMS * MAX_NUM_OF_ROTBONDS; m++) {
    KerConst_grads->rotbonds_atoms[m] = rotbonds_atoms[m];
  }
  for (m = 0; m < MAX_NUM_OF_ROTBONDS; m++) {
    KerConst_grads->num_rotating_atoms_per_rotbond[m] =
        num_rotating_atoms_per_rotbond[m];
  }
...
// number_or_req_rotations and element i of atom_id_of_numrots correspond to
// each other). Element i of the former array stores how many rotations have to
// be perfomed on the atom whose atom ID is stored by element i of the latter
// one. The third parameter has to be equal to the number of ligand atoms
{
  int i, j;
  int temp;
  ...
  /*	printf("\n\nRotation priority list after re-ordering:\n");
          for (i=0; i<num_of_atoms; i++)
                  printf("Roatom_rotbondstation of %d (required rots remaining:
     %d)\n", atom_id_of_numrots[i], number_of_req_rotations[i]);
          printf("\n\n");*/
}

int gen_rotlist(Liganddata* myligand, int rotlist[MAX_NUM_OF_ROTATIONS])
// The function generates the rotation list which will be stored in the constant
// memory field rotlist_const by prepare_const_fields_for_gpu(). The structure of
// this array is described at that function.
{
  ...
  for (atom_id = 0; atom_id < NUM_OF_THREADS_PER_BLOCK;
       atom_id++)  // handling special case when
                   // num_of_atoms<NUM_OF_THREADS_PER_BLOCK
  ...
  rotlist_id = 0;
  make_reqrot_ordering(number_of_req_rotations, atom_id_of_numrots,
                       myligand->num_of_atoms);
  while (number_of_req_rotations[0] !=
         0)  // if the atom with the most remaining rotations has to be rotated
             // 0 times, done
  {
    if (rotlist_id == MAX_NUM_OF_ROTATIONS) return 1;

    // putting the NUM_OF_THREADS_PER_BLOCK pieces of most important rotations
    // to the list
    for (parallel_rot_id = 0; parallel_rot_id < NUM_OF_THREADS_PER_BLOCK;
         parallel_rot_id++) {
      if (number_of_req_rotations[parallel_rot_id] ==
          0)  // if the atom has not to be rotated anymore, dummy rotation
        ...
        if (number_of_req_rotations[parallel_rot_id] == 1)
          ...
          remaining_rots_around_rotbonds =
              number_of_req_rotations[parallel_rot_id] -
              1;  //-1 because of genrot

          while (!rotbond_found) {
            if (myligand->atom_rotbonds[atom_id][rotbond_candidate] !=
                0)  // if the atom has to be rotated around current candidate
            {
              if (remaining_rots_around_rotbonds ==
                  1)  // if current value of remaining rots is 1, the proper
                      // rotbond is found
                rotbond_found = true;
              else
                remaining_rots_around_rotbonds--;  // if not, decresing
                                                   // remaining rots (that is,
                                                   // skipping rotations which
                                                   // have to be performed later
            }

            if (!rotbond_found) rotbond_candidate--;

            if (rotbond_candidate < 0) return 1;
          }

          new_rotlist_element |=
              (((int)rotbond_candidate) << RLIST_RBONDID_SHIFT) &
              RLIST_RBONDID_MASK;
        }
