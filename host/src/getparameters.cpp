...
int get_filelist(const int* argc, char** argv, FileList& filelist)
// The function checks if a filelist has been provided according to the proper
// command line arguments. If it is, it loads the .fld, .pdbqt, and resname files
// into vectors
{
  ...
  if (filelist.used) {
    ...
    while (std::getline(file, line)) {
      ...
      if (len >= 4 && line.compare(len - 4, 4, ".fld") == 0) {
        if (prev_line_was_fld) {  // Overwrite the previous fld file if two in a
                                  // row
        ...
        if (filelist.fld_files.size() == 0) {
          printf(
              "\nError: No map file on record yet. Please specify a .fld file "
              "before the first ligand (%s).\n",
              line.c_str());
          return 1;
        }
        if (filelist.ligand_files.size() > filelist.fld_files.size()) {
          // If this ligand doesnt have a protein preceding it, use the previous
          // protein
          filelist.fld_files.push_back(
              filelist.fld_files[filelist.fld_files.size() - 1]);
        }
    ...
    if (filelist.ligand_files.size() == 0) {
      printf(
          "\nError: No ligands, through lines ending with the .pdbqt suffix, "
          "have been specified.\n");
      return 1;
    }
    if ((filelist.ligand_files.size() != filelist.resnames.size()) &&
        (filelist.resnames.size() > 0)) {
      printf(
          "\nError: Inconsistent number of resnames (%lu) compared to ligands "
          "(%lu)!\n",
...
// according to the proper command line arguments.
{
  ...
  // this model assumes the BOUND conformation is the SAME as the UNBOUND,
  // default in AD4.2
  const AD4_free_energy_coeffs coeffs_bound = {
      0.1662, 0.1209, coeff_elec_scale_factor * 0.1406, 0.1322, 0.2983};

  // this model assumes the unbound conformation is EXTENDED, default if AD4.0
  const AD4_free_energy_coeffs coeffs_extended = {
      0.1560, 0.0974, coeff_elec_scale_factor * 0.1465, 0.1159, 0.2744};

  // this model assumes the unbound conformation is COMPACT
  const AD4_free_energy_coeffs coeffs_compact = {
      0.1641, 0.0531, coeff_elec_scale_factor * 0.1272, 0.0603, 0.2272};
...
void get_commandpars(const int* argc, char** argv, double* spacing,
                     Dockpars* mypars)
// The function processes the command line arguments given with the argc and
// argv parameters, and fills the proper fields of mypars according to that. If a
// parameter was not defined in the command line, the default value will be
// assigned. The mypars' fields will contain the data in the same format as it is
// required for writing it to algorithm defined registers.
{
  int i;
  long tempint;
  float tempfloat;
  ...
  mypars->heuristics_max =
      50000000;  // Maximum number of evaluations under the heuristics (50M
                 // evaluates to 80% at 12.5M evals calculated by heuristics)
  ...
  strcpy(mypars->xrayligandfile,
         mypars->ligandfile);  // By default xray-ligand file is the same as the
                               // randomized input ligand
  mypars->given_xrayligandfile =
      false;                  // That is, not given (explicitly by the user)
  ...
  for (i = 1; i < (*argc) - 1; i += 2) {
    ...
    if (strcmp("-nev", argv[i]) == 0) {
      ...
      if ((tempint > 0) && (tempint < 260000000)) {
        ...
        printf(
            "Warning: value of -nev argument ignored. Value must be between 0 "
            "and 260000000.\n");
    }
    ...
    if (strcmp("-ngen", argv[i]) == 0) {
      ...
      if ((tempint > 0) && (tempint < 16250000))
        ...
        printf(
            "Warning: value of -ngen argument ignored. Value must be between 0 "
            "and 16250000.\n");
    }
    ...
    if (strcmp("-initswgens", argv[i]) == 0) {
      ...
      if ((tempint >= 0) && (tempint <= 16250000))
        ...
        printf(
            "Warning: value of -initswgens argument ignored. Value must be "
            "between 0 and 16250000.\n");
    }

    // ----------------------------------
    // Argument: Use Heuristics for number of evaluations (can be overwritten
    // with -nev)
    ...
    if (strcmp("-heurmax", argv[i]) == 0) {
      ...
      if ((tempint > 0) && (tempint < 16250000))
        ...
        printf(
            "Warning: value of -heurmax argument ignored. Value must be "
            "between 0 and 16250000.\n");
    }

    // Argument: maximal delta movement during mutation. Must be an integer
    // between 1 and 16. N means that the maximal delta movement will be +/-
    // 2^(N-10)*grid spacing angström.
    if (strcmp("-dmov", argv[i]) == 0) {
      ...
      if ((tempfloat > 0) && (tempfloat < 10))
        ...
        printf(
            "Warning: value of -dmov argument ignored. Value must be a float "
            "between 0 and 10.\n");
    }

    // Argument: maximal delta angle during mutation. Must be an integer between
    // 1 and 17. N means that the maximal delta angle will be +/- 2^(N-8)*180/512
    // degrees.
    if (strcmp("-dang", argv[i]) == 0) {
      ...
      if ((tempfloat > 0) && (tempfloat < 180))
        ...
        printf(
            "Warning: value of -dang argument ignored. Value must be a float "
            "between 0 and 180.\n");
    }
    ...
    if (strcmp("-mrat", argv[i]) == 0) {
      ...
      if ((tempfloat >= 0.0) && (tempfloat < 100.0))
        ...
        printf(
            "Warning: value of -mrat argument ignored. Value must be a float "
            "between 0 and 100.\n");
    }
    ...
    if (strcmp("-crat", argv[i]) == 0) {
      ...
      if ((tempfloat >= 0.0) && (tempfloat <= 100.0))
        ...
        printf(
            "Warning: value of -crat argument ignored. Value must be a float "
            "between 0 and 100.\n");
    }
...
if ((tempfloat >= 0.0) && (tempfloat < 100.0))
      */
      if ((tempfloat >= 0.0) && (tempfloat <= 100.0))
        ...
        printf(
            "Warning: value of -lrat argument ignored. Value must be a float "
            "between 0 and 100.\n");
    }
    ...
    if (strcmp("-smooth", argv[i]) == 0) {
      ...
      if ((tempfloat >= 0.0f) && (tempfloat <= 0.5f))
        ...
        printf(
            "Warning: value of -smooth argument ignored. Value must be a float "
            "between 0 and 0.5.\n");
    }
    ...
    if (strcmp("-lsmet", argv[i]) == 0) {
      ...
      if (strcmp(temp, "sw") == 0) {
        ...
        printf(
            "Error: Value of -lsmet must be a valid string: \"sw\", \"sd\", "
            "\"fire\", \"ad\", or \"adam\".\n");
        exit(-1);
      }
    }
    ...
    // Means the probability that the better entity wins the tournament round
    // during selectin
    if (strcmp("-trat", argv[i]) == 0) {
      ...
      if ((tempfloat >= /*5*/ 0.0) && (tempfloat <= 100.0))
        ...
        printf(
            "Warning: value of -trat argument ignored. Value must be a float "
            "between 0 and 100.\n");
    }

    // Argument: rho lower bound. Must be a float between 0 and 1.
    // Means the lower bound of the rho parameter (possible stop condition for
    // local search).
    if (strcmp("-rholb", argv[i]) == 0) {
      ...
      if ((tempfloat >= 0.0) && (tempfloat < 1.0))
        ...
        printf(
            "Warning: value of -rholb argument ignored. Value must be a float "
            "between 0 and 1.\n");
    }

    // Argument: local search delta movement. Must be a float between 0 and grid
    // spacing*64 A. Means the spread of unifily distributed delta movement of
    // local search.
    if (strcmp("-lsmov", argv[i]) == 0) {
      ...
      if ((tempfloat > 0.0) && (tempfloat < (*spacing) * 64 / sqrt(3.0)))
        ...
        printf(
            "Warning: value of -lsmov argument ignored. Value must be a float "
            "between 0 and %lf.\n",
            64 * (*spacing));
    }
    ...
    if (strcmp("-lsang", argv[i]) == 0) {
      ...
      if ((tempfloat > 0.0) && (tempfloat < 103.0))
        ...
        printf(
            "Warning: value of -lsang argument ignored. Value must be a float "
            "between 0 and 103.\n");
    }

    // Argument: consecutive success/failure limit. Must be an integer between 1
    // and 255. Means the number of consecutive successes/failures after which
    // value of rho have to be doubled/halved.
    if (strcmp("-cslim", argv[i]) == 0) {
      ...
      if ((tempint > 0) && (tempint < 256))
        ...
        printf(
            "Warning: value of -cslim argument ignored. Value must be an "
            "integer between 1 and 255.\n");
    }

    // Argument: maximal number of iterations for local search. Must be an
    // integer between 1 and 262143. Means the number of iterations after which
    // the local search algorithm has to terminate.
    if (strcmp("-lsit", argv[i]) == 0) {
      ...
      if ((tempint > 0) && (tempint < 262144))
        ...
        printf(
            "Warning: value of -lsit argument ignored. Value must be an "
            "integer between 1 and 262143.\n");
    }

    // Argument: size of population. Must be an integer between 32 and
    // CPU_MAX_POP_SIZE. Means the size of the population in the genetic
    // algorithm.
    if (strcmp("-psize", argv[i]) == 0) {
      ...
      if ((tempint >= 2) && (tempint <= MAX_POPSIZE))
        ...
        printf(
            "Warning: value of -psize argument ignored. Value must be an "
            "integer between 2 and %d.\n",
            MAX_POPSIZE);
    }

    // Argument: load initial population from file instead of generating one.
    // If the value is zero, the initial population will be generated randomly,
    // otherwise it will be loaded from a file.
    ...
    // Argument: number of the initial population file.
    ...
    if (strcmp("-npdb", argv[i]) == 0) {
      ...
      if ((tempint < 0) || (tempint > MAX_POPSIZE))
        printf(
            "Warning: value of -npdb argument ignored. Value must be an "
    ...
    if (strcmp("-filelist", argv[i]) == 0) arg_recognized = 1;
    ...
    if (strcmp("-ffile", argv[i]) == 0) arg_recognized = 1;
    ...
    if (strcmp("-lfile", argv[i]) == 0) arg_recognized = 1;
    ...
    if (strcmp("-asfreq", argv[i]) == 0) {
      ...
      if ((tempint >= 1) && (tempint <= 100))
        ...
        printf(
            "Warning: value of -asfreq argument ignored. Value must be an "
            "integer between 1 and 100.\n");
    }
    // ----------------------------------

    // ----------------------------------
    // Argument: Stopping criterion standard deviation.. Must be a float between
    // 0.01 and 2.0; Means the energy standard deviation of the best candidates
    // after which to stop evaluation when autostop is 1..
    if (strcmp("-stopstd", argv[i]) == 0) {
      ...
      if ((tempfloat >= 0.01) && (tempfloat < 2.0))
        ...
        printf(
            "Warning: value of -stopstd argument ignored. Value must be a "
            "float between 0.01 and 2.0.\n");
    }
    // ----------------------------------
    ...
    if (strcmp("-nrun", argv[i]) == 0) {
      ...
      if ((tempint >= 1) && (tempint <= MAX_NUM_OF_RUNS))
        ...
        printf(
            "Warning: value of -nrun argument ignored. Value must be an "
            "integer between 1 and %d.\n",
            MAX_NUM_OF_RUNS);
    }

    // Argument: energies of reference ligand required.
    // If the value is not zero, energy values of the reference ligand is
    // required.
    ...
    // ---------------------------------
    // Argument: unbound model to be used.
    if (strcmp("-ubmod", argv[i]) == 0) arg_recognized = 1;
    ...
    // If the value is not zero, molecular syymetry will be taken into account
    // during rmsd calculation and clustering.
    ...
    // If the value is zero, result files containing the final populations won't
    // be generatied, otherwise they will.
    ...
    // If the value is zero, best.pdbqt file containing the coordinates of the
    // best result found during all of the runs won't be generated, otherwise it
    // will
    ...
    // Argument: use modified QASP (from VirtualDrug) instead of original one
    // used by AutoDock If the value is not zero, the modified parameter will be
    // used.
    ...
    if (strcmp("-rmstol", argv[i]) == 0) {
      ...
      if (tempfloat > 0.0)
        ...
        printf(
            "Warning: value of -rmstol argument ignored. Value must be a "
  ...
  if (mypars->pop_size < mypars->gen_pdbs) {
    printf(
        "Warning: value of -npdb argument igonred. Value mustn't be greater "
        "than the population size.\n");
    mypars->gen_pdbs = 1;
  }
}

void gen_initpop_and_reflig(Dockpars* mypars, float* init_populations,
                            float* ref_ori_angles, Liganddata* myligand,
...
// two allocated memory regions with proper size which the function will fill
// with random values. Each contiguous GENOTYPE_LENGTH_IN_GLOBMEM pieces of
// floats in init_population corresponds to a genotype, and each contiguous three
// pieces of floats in ref_ori_angles corresponds to the phi, theta and angle
// genes of the reference orientation. In addition, as part of reference
// orientation handling, the function moves myligand to origo and scales it
// according to grid spacing.
{
  ...
  if (mypars->initpop_gen_or_loadfile) {
    if (mypars->num_of_runs != 1) {
      printf(
          "Warning: more than 1 run was requested. New populations will be "
          "generated \ninstead of being loaded from %.*s\n",
          (int)sizeof(mypars->initpop_filename), mypars->initpop_filename);

      gen_pop = 1;
    } else {
      fp = fopen(mypars->initpop_filename, ,
                 "rb");  // fp = fopen("initpop.txt","r");
      if (fp == NULL) {
        printf(
            "Warning: can't find %.*s. A new population will be generated.\n",
            (int)sizeof(mypars->initpop_filename), mypars->initpop_filename);
        gen_pop = 1;
      } else {
        for (entity_id = 0; entity_id < pop_size; entity_id++)
          for (gene_id = 0; gene_id < MAX_NUM_OF_ROTBONDS + 6; gene_id++)
            fscanf(fp, "%f",
                   &(init_populations[entity_id * GENOTYPE_LENGTH_IN_GLOBMEM +
                                      gene_id]));
  ...
  if (gen_pop == 1) {
    for (entity_id = 0; entity_id < pop_size * mypars->num_of_runs;
         entity_id++) {
      for (gene_id = 0; gene_id < 3; gene_id++) {
        init_populations[entity_id * GENOTYPE_LENGTH_IN_GLOBMEM + gene_id] =
            r.random_float() * (mygrid->size_xyz_angstr[gene_id]);
      }
      ...
      init_populations[entity_id * GENOTYPE_LENGTH_IN_GLOBMEM + 3] =
          phi / DEG_TO_RAD;
      init_populations[entity_id * GENOTYPE_LENGTH_IN_GLOBMEM + 4] =
          theta / DEG_TO_RAD;
      init_populations[entity_id * GENOTYPE_LENGTH_IN_GLOBMEM + 5] =
          rotangle / DEG_TO_RAD;

      // printf("angles = %8.2f, %8.2f, %8.2f\n", phi / DEG_TO_RAD, theta /
      // DEG_TO_RAD, rotangle/DEG_TO_RAD);

      /*
init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+3] = (float) myrand() *
360.0; init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+4] = (float)
myrand() * 360.0; init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+5] =
(float) myrand() * 360.0;
init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+3] = (float) myrand() *
360; init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+4] = (float) myrand()
* 180; init_populations[entity_id*GENOTYPE_LENGTH_IN_GLOBMEM+5] = (float)
myrand() * 360;
      */
  ...
  // genotypes should contain x, y and z genes in grid spacing instead of
  // Angstroms (but was previously generated in Angstroms since fdock does the
  //same)
...
movvec_to_origo[1], movvec_to_origo[2]);
  */
}
