...
int main(int argc, char* argv[]) {
    ...
    FileList filelist;
    int n_files;
    if (get_filelist(&argc, argv, filelist) != 0) return 1;
    if (filelist.used) {
    ...
    for (unsigned int i = 1; i < argc - 1; i += 2) {
        if (strcmp("-devnum", argv[i]) == 0) {
            ...
            if ((tempint >= 1) && (tempint <= 65536))
                ...
                printf(
                    "Warning: value of -devnum argument ignored. Value must be an "
...
#pragma omp for schedule(dynamic, 1)
            ...
            for (int i_job = 0; i_job < n_files; i_job++) {
                ...
                if (setup(all_maps, mygrid, floatgrids, mypars, myligand_init,
                          myxrayligand, filelist, i_job, argc, argv) != 0) {
                    // If error encountered: Set error flag to 1; Add to count of finished
                    // jobs Keep in setup stage rather than moving to launch stage so a
                    // different job will be set up
                    printf("\n\nError in setup of Job #%d:\n", i_job);
                    if (filelist.used) {
                        printf("(   Field file: %s )\n", filelist.fld_files[i_job].c_str());
                        printf("(   Ligand file: %s )\n",
                               filelist.ligand_files[i_job].c_str());
                        fflush(stdout);
                    }
                    err[i_job] = 1;
                    continue;
...
#pragma omp critical
#endif
                    {
                        if (filelist.preload_maps && filelist.load_maps_gpu) {
                            int size_of_one_map = 4 * mygrid.size_xyz[0] * mygrid.size_xyz[1] *
                                mygrid.size_xyz[2];
...
#pragma omp critical
#endif
                {
                    ...
                    // Dock
                    error_in_docking = docking_with_gpu(
                        &(mygrid), floatgrids.data(), &(mypars), &(myligand_init),
                        &(myxrayligand), profiler.p[(get_profiles ? i_job : 0)], &argc,
                        argv, sim_state, cData, tData, filelist.preload_maps);
                ...
                if (error_in_docking != 0) {
                    // If error encountered: Set error flag to 1; Add to count of finished
                    // jobs Set back to setup stage rather than moving to processing stage
                    // so a different job will be set up
...
#pragma omp atomic update
                ...
                process_result(&(mygrid), floatgrids.data(), &(mypars), &(myligand_init),
                               &(myxrayligand), &argc, argv, sim_state);
#ifdef USE_PIPELINE
#pragma omp atomic update
#endif
        ...
        printf("\nRun time of entire job set (%d files): %.3f sec", n_files,
               seconds_since(time_start));
        printf("\nSavings from multithreading: %.3f sec",
               (total_setup_time + total_processing_time + total_exec_time) -
               seconds_since(time_start));
        // if (filelist.preload_maps) printf("\nSavings from receptor reuse: %.3f sec
        // * avg_maps_used/n_maps",receptor_reuse_time*n_files);
        printf("\nIdle time of execution thread: %.3f sec",
               seconds_since(time_start) - total_exec_time);
        if (get_profiles && filelist.used)
            profiler.write_profiles_to_file(filelist.filename);
#endif
