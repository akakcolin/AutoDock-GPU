...
// The function converts a float value to a fixed pont fractional number in
// (32-frac).frac format, and returns it as an integer.
{
  if (toconv >= 0)
...
// The function converts a float value to a fixed pont fractional number in
// (32-frac).frac format, and returns it as a long integer.
{
  if (toconv >= 0)
...
//The function returns the current time in seconds.
{
  ...
  timesec = ((double) ts.tv_sec*1000000000.0 + (double)
ts.tv_usec*1000.0)/1000000000.0; return timesec;
}*/
...
void vec_point2line(const double point[], const double line_pointA[],
                    const double line_pointB[], double vec[])
// The function calculates the vector which moves a point given by the first
// parameter to its perpendicular projection on a line given by to of its points
// (line_pointA and line_pointB parameters). The result vector is the vec
// parameter.
{
  ...
  for (i = 0; i < 3; i++) posvec_of_line[i] = line_pointB[i] - line_pointA[i];
  ...
  for (i = 0; i < 3; i++) vec[i] = proj_of_point[i] - point[i];
}

void rotate(double point[], const double movvec[], const double normvec[],
            const double* angle, int debug)
...
// according to right hand rule. If debug is 1, debug messages will be printed
// to the screen.
{
  ...
  if (debug == 1) {
    printf("q (w,x,y,z): %lf, %lf, %lf, %lf\n", quatrot_left.q, quatrot_left.x,
           quatrot_left.y, quatrot_left.z);
    printf("q^-1 (w,x,y,z): %lf, %lf, %lf, %lf\n", quatrot_right.q,
           quatrot_right.x, quatrot_right.y, quatrot_right.z);
  ...
  quatrot_temp.q = 0 - quatrot_left.x * point[0] - quatrot_left.y * point[1] -
                   quatrot_left.z * point[2];
  quatrot_temp.x = quatrot_left.q * point[0] + 0 + quatrot_left.y * point[2] -
                   quatrot_left.z * point[1];
  quatrot_temp.y = quatrot_left.q * point[1] - quatrot_left.x * point[2] + 0 +
                   quatrot_left.z * point[0];
  quatrot_temp.z = quatrot_left.q * point[2] + quatrot_left.x * point[1] -
                   quatrot_left.y * point[0] + 0;

  if (debug == 1)
    printf("q*v (w,x,y,z): %lf, %lf, %lf, %lf\n", quatrot_temp.q,
           quatrot_temp.x, quatrot_temp.y, quatrot_temp.z);

  point[0] =
      quatrot_temp.q * quatrot_right.x + quatrot_temp.x * quatrot_right.q +
      quatrot_temp.y * quatrot_right.z - quatrot_temp.z * quatrot_right.y;
  point[1] =
      quatrot_temp.q * quatrot_right.y - quatrot_temp.x * quatrot_right.z +
      quatrot_temp.y * quatrot_right.q + quatrot_temp.z * quatrot_right.x;
  point[2] =
      quatrot_temp.q * quatrot_right.z + quatrot_temp.x * quatrot_right.y -
      quatrot_temp.y * quatrot_right.x + quatrot_temp.z * quatrot_right.q;
...
// The function's inputs are two position vectors (whose starting point is the
// origo). The function returns the angle between them.
{
  int i;
  ...
  for (i = 0; i < 3; i++) scalmul += vector1[i] * vector2[i];
...
void vec_crossprod(const double vector1[], const double vector2[],
                   double crossprodvec[])
// The function calculates the cross product of position vectors vector1 and
// vector2, and returns it in the third parameter.
{
...
void get_trilininterpol_weights(double weights[][2][2], const double* dx,
                                const double* dy, const double* dz)
// The function calculates the weights for trilinear interpolation based on the
// location of the point inside the cube which is given by the second, third and
// fourth parameters.
{
...
void get_trilininterpol_weights_f(float weights[][2][2], const float* dx,
...
// The function prints out the value of to_print parameter to the standart io as
// a binary number.
{
...
// and 1 if not.
{
  ...
  if (toupper(c1) != toupper(c2)) isdifferent = 1;
