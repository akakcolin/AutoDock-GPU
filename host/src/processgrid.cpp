...
int get_gridinfo(const char* fldfilename, Gridinfo* mygrid) {
  ...
  while (fscanf(fp, "%s", tempstr) != EOF) {
    ...
    if (strcmp(tempstr, "#NELEMENTS") == 0) {
      ...
      // If the grid is too big, send message and change the value of
      // truncated_size_xyz
      if ((mygrid->size_xyz[0] > MAX_NUM_GRIDPOINTS) ||
          (mygrid->size_xyz[1] > MAX_NUM_GRIDPOINTS) ||
          (mygrid->size_xyz[2] > MAX_NUM_GRIDPOINTS)) {
...
int get_gridvalues_f(const Gridinfo* mygrid, float** fgrids, bool cgmaps) {
  *fgrids = (float*)malloc(4 * (sizeof(float)) * (mygrid->num_of_atypes + 2) *
                           (mygrid->size_xyz[0]) * (mygrid->size_xyz[1]) *
                           (mygrid->size_xyz[2]));
  if (*fgrids == NULL) {
