...
void arrange_result(float* final_population, float* energies,
                    const int pop_size)
// The function arranges the rows of the input array (first array index is
// considered to be the row index) according to the sum of [] [38] and [][39]
// elements, which can be used for arranging the genotypes of the final
// population according to the sum of energy values. Genotypes with lower
// energies will be placed at lower row indexes. The second parameter must be
// equal to the size of the population, the arrangement will be performed only on
// the first pop_size part of final_population.
{
  int i, j;
  ...
  for (j = 0; j < pop_size - 1; j++)
    for (i = pop_size - 2; i >= j;
         i--)  // arrange according to sum of inter- and intramolecular energies
      if (energies[i] > energies[i + 1]) {
        memcpy(temp_genotype, final_population + i * GENOTYPE_LENGTH_IN_GLOBMEM,
               GENOTYPE_LENGTH_IN_GLOBMEM * sizeof(float));
        memcpy(final_population + i * GENOTYPE_LENGTH_IN_GLOBMEM,
               final_population + (i + 1) * GENOTYPE_LENGTH_IN_GLOBMEM,
               GENOTYPE_LENGTH_IN_GLOBMEM * sizeof(float));
        memcpy(final_population + (i + 1) * GENOTYPE_LENGTH_IN_GLOBMEM,
               temp_genotype, GENOTYPE_LENGTH_IN_GLOBMEM * sizeof(float));
...
void write_basic_info(FILE* fp, const Liganddata* ligand_ref,
                      const Dockpars* mypars, const Gridinfo* mygrid,
                      const int* argc, char** argv)
// The function writes basic information (such as docking parameters) to the
// file whose file pointer is the first parameter of the function.
{
  ...
  if (mypars->unbound_model == 0)
    fprintf(fp, "BOUND\n");
  else if (mypars->unbound_model == 1)
    fprintf(fp, "EXTENDED\n");
  else
    fprintf(fp, "COMPACT\n");
  ...
  fprintf(fp, "Grid size (x, y, z):                       %lf, %lf, %lfA\n",
          mygrid->size_xyz_angstr[0], mygrid->size_xyz_angstr[1],
          mygrid->size_xyz_angstr[2]);
...
void write_basic_info_dlg(FILE* fp, const Liganddata* ligand_ref,
                          const Dockpars* mypars, const Gridinfo* mygrid,
                          const int* argc, char** argv)
// The function writes basic information (such as docking parameters) to the
// file whose file pointer is the first parameter of the function.
...
void make_resfiles(float* final_population, float* energies,
                   const Liganddata* ligand_ref,
                   const Liganddata* ligand_from_pdb,
                   const Liganddata* ligand_xray, const Dockpars* mypars,
                   int evals_performed, int generations_used,
                   const Gridinfo* mygrid, const float* grids,
                   float* cpu_ref_ori_angles, const int* argc, char** argv,
                   int debug, int run_cnt, float& best_energy_of_all,
                   Ligandresult* best_result)
// The function writes out final_population generated by get_result
// as well as different parameters about the docking, the receptor and the
// ligand to a file called fdock_report.txt in a readable and understandable
// format. The ligand_from_pdb parametere must be the Liganddata which includes
// the original ligand conformation as the result conformations will be compared
// to this one. The structs containing the grid informations and docking
// parameters are requrided as well as the number and values of command line
// arguments. The ligand_ref parameter describes the ligand with the reference
// orientation (gene values of final_population refer to this one, that is, this
// can be moved and rotated according to the genotype values). The function
// returns some information about the best result wich was found with the
// best_result parameter.
{
  FILE* fp;
  int i, j;
  ...
  if (mypars->gen_finalpop)  // if final population files are not required, no
                             // file will be opened.
  {
    fp = fopen(temp_filename, "w");

    write_basic_info(fp, ligand_ref, mypars, mygrid, argc,
                     argv);  // Write basic information about docking and
                             // molecule parameters to file
  ...
  name_ext_start =
      temp_filename + strlen(mypars->ligandfile) - 6;  // without .pdbqt

  IntraTables tables(ligand_ref, mypars->coeffs.scaled_AD4_coeff_elec,
                     mypars->coeffs.AD4_coeff_desolv, mypars->qasp);
  for (i = 0; i < pop_size; i++) {
    temp_docked = *ligand_ref;

    // if (i==127)
    //	change_conform(&temp_docked, final_population [i], 1);
    ////calculating the conformation of current entity else
    change_conform_f(&temp_docked,
                     final_population + i * GENOTYPE_LENGTH_IN_GLOBMEM,
                     cpu_ref_ori_angles, debug);

    // if (i==78)
    //	accurate_interE [i] = calc_interE(mygrid, &temp_docked, grids, 0.00, 1);
    // else
    accurate_interE[i] =
        calc_interE_f(mygrid, &temp_docked, grids, 0.00,
                      debug);  // calculating the intermolecular energy

    if (i == 0)  // additional calculations for ADT-compatible result file, only
                 // in case of best conformation
      calc_interE_peratom_f(
          mygrid, &temp_docked, grids, 0.00, &(best_result->interE_elec),
          best_result->peratom_vdw, best_result->peratom_elec, debug);

    scale_ligand(&temp_docked, mygrid->spacing);
    // if (i==127)
    //	accurate_intraE [i] = calc_intraE(&temp_docked, 8, 0,
    //mypars->coeffs.scaled_AD4_coeff_elec, mypars->coeffs.AD4_coeff_desolv, 1);
    ////calculating the intramolecular energy else
#if 0
			accurate_intraE[i] = calc_intraE_f(&temp_docked, 8, 0, tables, debug);
#endif
    accurate_intraE[i] =
        calc_intraE_f(&temp_docked, 8, mypars->smooth, 0, tables, debug);

    move_ligand(
        &temp_docked,
        mygrid->origo_real_xyz);  // moving it according to grid location
    if (mypars->given_xrayligandfile == true) {
      entity_rmsds[i] =
          calc_rmsd(ligand_xray, &temp_docked,
                    mypars->handle_symmetry);  // calculating rmds compared to
                                               // original xray file
    } else {
      entity_rmsds[i] =
          calc_rmsd(ligand_from_pdb, &temp_docked,
                    mypars->handle_symmetry);  // calculating rmds compared to
                                               // original pdb file
    }

    // copying best result to output parameter
    if (i == 0)  // assuming this is the best one (final_population is
                 // arranged), however,
    {  // arrangement was made according to the unaccurate values calculated by
       // FPGA
    ...
    if (i < mypars->gen_pdbs)  // if it is necessary, making new pdbs for best
                               // entities
    {
      sprintf(name_ext_start, "_docked_run%d_entity%d.pdbqt", run_cnt + 1,
              i + 1);  // name will be <original pdb filename>_docked_<number
                       // starting from 1>.pdb
  ...
  if (mypars->gen_finalpop) {
    ...
    fprintf(fp,
            " Entity |      dx [A]      |      dy [A]      |      dz [A]      "
            "|     phi []      |    theta []     | alpha_genrot [] |");
    for (i = 0; i < ligand_from_pdb->num_of_rotbonds; i++)
      fprintf(fp, " alpha_rotb%2d [] |", i);
    fprintf(
        fp,
        " intramolecular energy | intermolecular energy | total energy "
        "calculated by CPU / calculated by GPU / difference | RMSD [A] | \n");
    ...
    for (i = 0; i < pop_size; i++) {
      ...
      for (j = 0; j < 3; j++)
        fprintf(fp, "    %10.3f    |",
                final_population[i * GENOTYPE_LENGTH_IN_GLOBMEM + j] *
                    (mygrid->spacing));
      ...
      fprintf(fp, "  %21.3f / %21.3f / %21.3f |",
              accurate_intraE[i] + accurate_interE[i], energies[i],
              energies[i] - (accurate_intraE[i] + accurate_interE[i]));
...
void cluster_analysis(Ligandresult myresults[], int num_of_runs,
                      char* report_file_name, const Liganddata* ligand_ref,
                      const Dockpars* mypars, const Gridinfo* mygrid,
                      const int* argc, char** argv,
                      const double docking_avg_runtime,
                      const double program_runtime)
// The function performs ranked cluster analisys similar to that of AutoDock and
// creates a file with report_file_name name, the result will be written to it.
{
  int i, j;
  ...
  // arranging results according to energy, myresults [0] will be the best one
  // (with lowest energy)
  for (j = 0; j < num_of_runs - 1; j++)
    for (i = num_of_runs - 2; i >= j;
         i--)  // arrange according to sum of inter- and intramolecular energies
      if ((myresults[i]).interE /*+ (myresults [i]).intraE*/ >
          (myresults[i + 1])
              .interE /*+ (myresults [i+1]).intraE*/)  // mimics the behaviour
                                                       // of AD4
                                                       // unbound_same_as_bound
      // if ((myresults [i]).interE + (myresults [i]).intraE > (myresults
      // [i+1]).interE + (myresults [i+1]).intraE)
      {
  ...
  for (i = 0; i < num_of_runs; i++) {
    (myresults[i]).clus_id =
        0;  // indicates that it hasn't been put into cluster yet
  }
  ...
  for (i = 1; i < num_of_runs; i++)  // for each result
  {
    ...
    for (j = 0; j < i; j++)  // results with lower id-s are clustered, look for
                             // cluster centers
    {
      if ((myresults[j]).clus_id >
          current_clust_center)  // it is the center of a new cluster
      {
        current_clust_center = (myresults[j]).clus_id;
        temp_rmsd =
            calc_rmsd(&((myresults[j]).reslig_realcoord),
                      &((myresults[i]).reslig_realcoord),
                      mypars->handle_symmetry);  // comparing current result
                                                 // with cluster center
        if (temp_rmsd <= cluster_tolerance)  // in this case we put result i to
                                             // cluster with center j
        {
    ...
    if (result_clustered !=
        1)  // if no suitable cluster was found, this is the center of a new one
    {
      num_of_clusters++;
  ...
  for (i = 1; i <= num_of_clusters; i++)  // printing cluster info to file
  {
    ...
    for (j = 0; j < num_of_runs; j++)
      if (myresults[j].clus_id == i) {
        subrank++;
        (cluster_sizes[i - 1])++;
        sum_energy[i - 1] += (myresults[j]).interE +
                             /*(myresults [j]).intraE +*/
                             torsional_energy;  // intraE can be commented when
                                                // unbound_same_as_bound
        (myresults[j]).clus_subrank = subrank;
        if (subrank == 1)
          best_energy[i - 1] = (myresults[j]).interE +
                               /*(myresults [j]).intraE +*/
                               torsional_energy;  // intraE can be commented
                                                  // when unbound_same_as_bound
      }
  }
  ...
  write_basic_info(fp, ligand_ref, mypars, mygrid, argc,
                   argv);  // Write basic information about docking and molecule
                           // parameters to file
  ...
  fprintf(fp, "===================================\n\n");
  fprintf(fp,
          " Cluster rank | Num in cluster |   Best energy   |   Mean energy   "
          "|    5    10   15   20   25   30   35\n");
  fprintf(fp,
          "--------------+----------------+-----------------+-----------------+"
          "----+----+----+----+----+----+----+\n");
  ...
  for (i = 1; i <= num_of_clusters; i++) {
    fprintf(fp, "      %3d     |       %3d      | %15.3lf | %15.3lf |", i,
            cluster_sizes[i - 1], best_energy[i - 1],
            sum_energy[i - 1] / cluster_sizes[i - 1]);

    for (j = 0; j < cluster_sizes[i - 1]; j++) fprintf(fp, "#");
  ...
  fprintf(
      fp,
      " Rank | Subrank | Run | Intermolecular E | Intramolecular E | Torsional "
      "energy |   Total energy   | Cluster RMSD | Reference RMSD |\n");
  fprintf(fp,
          "------+---------+-----+------------------+------------------+-------"
          "-----------+------------------+--------------+----------------+\n");
  ...
  for (i = 1; i <= num_of_clusters; i++)  // printing cluster info to file
  {
    for (j = 0; j < num_of_runs; j++)
      if (myresults[j].clus_id == i) {
        fprintf(fp,
                "  %3d |   %3d   | %3d |  %15.3lf |  %15.3lf |  %15.3lf |  "
                "%15.3lf |     %4.2lf     |      %4.2lf      |\n",
                (myresults[j]).clus_id, (myresults[j]).clus_subrank,
                (myresults[j]).run_number, (myresults[j]).interE,
                (myresults[j]).intraE, torsional_energy,
                (myresults[j]).interE +
                    /*(myresults [j]).intraE +*/ torsional_energy,
                (myresults[j]).rmsd_from_cluscent,
                (myresults[j]).rmsd_from_ref);  // intraE can be commented when
                                                // unbound_same_as_bound
      }
  }
...
void clusanal_gendlg(Ligandresult myresults[], int num_of_runs,
                     const Liganddata* ligand_ref, const Dockpars* mypars,
                     const Gridinfo* mygrid, const int* argc, char** argv,
                     const double docking_avg_runtime,
                     unsigned long generations_used,
                     unsigned long evals_performed, double exec_time,
                     double idle_time)
// The function performs ranked cluster analisys similar to that of AutoDock and
// creates a file with report_file_name name, the result will be written to it.
{
  ...
  fp_orig = fopen(mypars->ligandfile,
                  "rb");  // fp_orig = fopen(mypars->ligandfile, "r");

  while (fgets(tempstr, 255, fp_orig) !=
         NULL)  // reading original ligand pdb line by line
  {
  ...
  for (i = 0; i < num_of_runs; i++) {
    ...
    while (fgets(tempstr, 255, fp_orig) !=
           NULL)  // reading original ligand pdb line by line
    {
      if ((strncmp("ATOM", tempstr, 4) == 0) ||
          (strncmp("HETATM", tempstr, 6) == 0)) {
        tempstr[30] = '\0';
        fprintf(fp, "DOCKED: %s", tempstr);
        fprintf(fp, "%8.3lf",
                myresults[i].reslig_realcoord.atom_idxyzq[atom_cnt][1]);  // x
        fprintf(fp, "%8.3lf",
                myresults[i].reslig_realcoord.atom_idxyzq[atom_cnt][2]);  // y
        fprintf(fp, "%8.3lf",
                myresults[i].reslig_realcoord.atom_idxyzq[atom_cnt][3]);  // z
        fprintf(fp, "%+6.2lf", myresults[i].peratom_vdw[atom_cnt]);       // vdw
        fprintf(fp, "%+6.2lf", myresults[i].peratom_elec[atom_cnt]);  // elec
        fprintf(fp, "    %+6.3lf ",
                myresults[i].reslig_realcoord.atom_idxyzq[atom_cnt][4]);  // q
        fprintf(fp, "%-2s\n",
                myresults[i].reslig_realcoord.atom_types[(
                    (int)myresults[i]
                        .reslig_realcoord.atom_idxyzq[atom_cnt][0])]);  // type
        atom_cnt++;
      } else if (strncmp("ROOT", tempstr, 4) == 0) {
        fprintf(fp,
                "DOCKED: USER                              x       y       z   "
                "  vdW  Elec       q    Type\n");
        fprintf(fp,
                "DOCKED: USER                           _______ _______ "
  ...
  fprintf(fp,
          "Rank | Energy    |     | Energy    | Clus|    5    10   15   20   "
          "25   30   35\n");
  fprintf(fp,
          "_____|___________|_____|___________|_____|____:____|____:____|____:_"
          "___|____:___\n");
  ...
  for (i = 0; i < num_of_clusters; i++) {
    ...
    for (j = 0; j < cluster_sizes[i]; j++) fprintf(fp, "#");

    fprintf(fp, "\n");
  }

  fprintf(fp,
          "_____|___________|_____|___________|_____|__________________________"
          "____________\n\n\n");
    ...
    for (j = 0; j < num_of_runs; j++)
      if (myresults[j].clus_id == i + 1) {
        if (myresults[j].interE + torsional_energy > 999999.99)
          fprintf(
              fp, "%4d   %4d   %4d  %+10.2e  %8.2f  %8.2f           RANKING\n",
              (myresults[j]).clus_id, (myresults[j]).clus_subrank,
              (myresults[j]).run_number, myresults[j].interE + torsional_energy,
              (myresults[j]).rmsd_from_cluscent, (myresults[j]).rmsd_from_ref);
        else
          fprintf(
              fp, "%4d   %4d   %4d  %+10.2f  %8.2f  %8.2f           RANKING\n",
              (myresults[j]).clus_id, (myresults[j]).clus_subrank,
              (myresults[j]).run_number, myresults[j].interE + torsional_energy,
              (myresults[j]).rmsd_from_cluscent, (myresults[j]).rmsd_from_ref);
      }
  ...
  if (mypars->output_xml == true) {
    ...
    for (i = 0; i < num_of_clusters; i++) {
      fprintf(
          fp_xml,
          "\t\t<cluster cluster_rank=\"%d\" lowest_binding_energy=\"%.2lf\" "
          "run=\"%d\" mean_binding_energy=\"%.2lf\" num_in_clus=\"%d\" />\n",
          i + 1, best_energy[i], best_energy_runid[i],
          sum_energy[i] / cluster_sizes[i], cluster_sizes[i]);
    }
    fprintf(fp_xml, "\t</clustering_histogram>\n");
    ...
    for (i = 0; i < num_of_clusters; i++) {
      for (j = 0; j < num_of_runs; j++)
        if (myresults[j].clus_id == i + 1) {
          fprintf(
              fp_xml,
              "\t\t<run rank=\"%d\" sub_rank=\"%d\" run=\"%d\" "
              "binding_energy=\"%.2lf\" cluster_rmsd=\"%.2lf\" "
              "reference_rmsd=\"%.2lf\" />\n",
              (myresults[j]).clus_id, (myresults[j]).clus_subrank,
              (myresults[j]).run_number, myresults[j].interE + torsional_energy,
              (myresults[j]).rmsd_from_cluscent, (myresults[j]).rmsd_from_ref);
...
void process_result(const Gridinfo* mygrid, const float* cpu_floatgrids,
                    const Dockpars* mypars, const Liganddata* myligand_init,
                    const Liganddata* myxrayligand, const int* argc,
                    char** argv, SimulationState& sim_state) {
  ...
  for (unsigned long run_cnt = 0; run_cnt < mypars->num_of_runs; run_cnt++) {
    arrange_result(sim_state.cpu_populations.data() +
                       run_cnt * mypars->pop_size * GENOTYPE_LENGTH_IN_GLOBMEM,
                   sim_state.cpu_energies.data() + run_cnt * mypars->pop_size,
                   mypars->pop_size);
    make_resfiles(sim_state.cpu_populations.data() +
                      run_cnt * mypars->pop_size * GENOTYPE_LENGTH_IN_GLOBMEM,
                  sim_state.cpu_energies.data() + run_cnt * mypars->pop_size,
                  &(sim_state.myligand_reference), myligand_init, myxrayligand,
                  mypars, sim_state.cpu_evals_of_runs[run_cnt],
                  sim_state.generation_cnt, mygrid, cpu_floatgrids,
                  sim_state.cpu_ref_ori_angles.data() + 3 * run_cnt, argc, argv,
                  /*1*/ 0, run_cnt, best_energy_of_all,
                  &(cpu_result_ligands[run_cnt]));
  }

  // Do clustering analysis and generate dlg file
  clusanal_gendlg(cpu_result_ligands.data(), mypars->num_of_runs, myligand_init,
                  mypars, mygrid, argc, argv, sim_state.sec_per_run,
