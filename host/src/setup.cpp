...
int preload_gridsize(FileList& filelist) {
  ...
  for (unsigned int i_file = 0; i_file < filelist.fld_files.size(); i_file++) {
    ...
    int curr_size =
        4 * mygrid.size_xyz[0] * mygrid.size_xyz[1] * mygrid.size_xyz[2];
    if (curr_size > gridsize) gridsize = curr_size;
  }
  return gridsize;
}

int setup(std::vector<Map>& all_maps, Gridinfo& mygrid,
          std::vector<float>& floatgrids, Dockpars& mypars,
          Liganddata& myligand_init, Liganddata& myxrayligand,
          FileList& filelist, int i_file, int argc, char* argv[]) {
  //------------------------------------------------------------
  ...
  // Filling the filename and coeffs fields of mypars according to command line
  // arguments
  ...
  //------------------------------------------------------------
  mypars.cgmaps = 0;  // default is 0 (use one maps for every CGx or Gx atom
                      // types, respectively)
  ...
  if (init_liganddata(mypars.ligandfile, &myligand_init, &mygrid,
                      mypars.cgmaps) != 0) {
  ...
  if (get_liganddata(mypars.ligandfile, &myligand_init,
                     mypars.coeffs.AD4_coeff_vdW,
                     mypars.coeffs.AD4_coeff_hb) != 0) {
    printf("\n\nError in get_liganddata, stopped job.");
    return 1;
  }

  // Resize grid
  floatgrids.resize(4 * (mygrid.num_of_atypes + 2) * mygrid.size_xyz[0] *
...
#pragma omp critical
#endif
      {
        if (!filelist.maps_are_loaded) {  // maps not yet loaded (but in
                                          // critical, so only one thread will
                                          // ever enter this)
          // Load maps to all_maps
          if (load_all_maps(mypars.fldfile, &mygrid, all_maps, mypars.cgmaps) !=
              0) {
            got_error = true;
          }
          filelist.maps_are_loaded = true;
          filelist.load_maps_gpu =
              true;  // first thread seeing this will copy to GPU
        }
      }
    ...
    // Map atom_types used for ligand processing to all_maps so all the maps can
    // stay on GPU
    ...
    // Reading the grid files and storing values in the memory region pointed by
    // floatgrids
  ...
  if (filelist.resnames.size() > 0) {  // Overwrite resname with specified
                                       // filename if specified in file list
    strcpy(mypars.resname, filelist.resnames[i_file].c_str());
  } else if (filelist.used) {  // otherwise add the index to existing name
                               // distinguish the files if multiple
  ...
  if (mypars.given_xrayligandfile == true) {
    if (init_liganddata(mypars.xrayligandfile, &myxrayligand, &mydummygrid,
                        mypars.cgmaps) != 0) {
      printf("\n\nError in init_liganddata, stopped job.");
      return 1;
    }

    if (get_liganddata(mypars.xrayligandfile, &myxrayligand,
                       mypars.coeffs.AD4_coeff_vdW,
                       mypars.coeffs.AD4_coeff_hb) != 0) {
      printf("\n\nError in get_liganddata, stopped job.");
      return 1;
  ...
  if (mypars.reflig_en_required) {
    print_ref_lig_energies_f(myligand_init, mypars.smooth, mygrid,
                             floatgrids.data(),
...
int fill_maplist(const char* fldfilename, std::vector<Map>& all_maps) {
  ...
  if (file.fail()) {
    printf("\nError: Could not open %s. Check path and permissions.",
           fldfilename);
    return 1;
  }
  std::string line;
  ...
  while (std::getline(file, line)) {
    ...
    while (std::getline(sline, word, ' ')) {
      ...
      int len = word.size();
      if (is_variable_line && len >= 4 &&
          word.compare(len - 4, 4, ".map") ==
              0) {  // Found a word that ends in "map"
        // Split the map into segments e.g. protein.O.map -> "protein", "O",
        // "map"
...
int load_all_maps(const char* fldfilename, const Gridinfo* mygrid,
                  std::vector<Map>& all_maps, bool cgmaps) {
  ...
  for (t = 0; t < all_maps.size(); t++) {
    ...
    if (fp == NULL) {
      ...
      if ((strncmp(all_maps[t].atype.c_str(), "CG", 2) == 0) ||
        ...
        if (cgmaps)
          printf(
              "-> Expecting an individual map for each CGx and Gx (x=0..9) "
              "atom type.\n");
        else
          printf(
              "-> Expecting one map file, ending in .CG.map and .G0.map, for "
              "CGx and Gx atom types, respectively.\n");
      }
      return 1;
    }
...
int copy_from_all_maps(const Gridinfo* mygrid, float* fgrids,
  ...
  for (int t = 0; t < mygrid->num_of_atypes + 2; t++) {
    ...
    if (i_map == -1) {  // Didnt find the map
      printf(
          "\nError: The %s map needed for the ligand was not found in the .fld "
          "file!",
          mygrid->grid_types[t]);
      return 1;
    }
    ...
    memcpy(fgrids + t * size_of_one_map, all_maps[i_map].grid.data(),
           sizeof(float) * all_maps[i_map].grid.size());
  }
